...
/**
 * Exercises the type-driven read-layer formatter end-to-end: the engine renders results through
 * {@link LocaleFormat} in canonical (ISO) mode, so output is driven by each column's resolved
 * TypeDescriptor — integers without a forced ".0", decimals at their real scale, dates as ISO.
 * Distinct from the StableFormat suites (which keep the formatter inert to isolate SQL goldens).
 */
public class CanonicalFormatDBEngineTest extends BaseEngineTest<HeaderInfo> {
    ...
    protected Path queriesRoot() {
        return Path.of("src/test/resources/ai/koryki/duckdb/queries/format");
    }

    @Override
    protected Path expectedCsv() {
        return Path.of("src/test/resources/ai/koryki/duckdb/expected/format/csv");
    }

    @Override
    protected Path expectedSql() {
        return Path.of("src/test/resources/ai/koryki/duckdb/expected/format/sql");
    }
