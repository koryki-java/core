...
/**
 * DuckDB has a single INTERVAL type (months + days + microseconds). Its JDBC driver has no interval
 * value class, so any INTERVAL value — a native interval column or a bare duration
 * literal/expression rendered with {@code to_years(...)} etc. — reads back as the verbose string
 * DuckDB renders, e.g. {@code "1 year 2 months 3 days 04:05:06.789"}, {@code "4 days 05:06:07"} or
 * {@code "00:01:30"}. Parse that to the canonical {@link Interval}.
 *
 * <p>The only INTERVAL-family values <em>not</em> read as a DuckDB verbose string are the
 * non-native encodings — a numeric count ({@code INTERVAL:<unit>}) or an ISO/clock text ({@code
 * INTERVAL_FROM_STRING}) — which {@link CoreDecoder} decodes instead.
 */
