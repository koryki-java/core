...
4. rewrite `BETWEEN` with a temporal upper bound into the half-open form
   `>= lower AND < upper + 1d`, so the upper day is included as a whole
   (see [TEMPORAL.md](TEMPORAL.md)); this happens on the way from **IQL** to **SQL**
