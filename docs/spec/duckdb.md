...
# Function & operator specification: duckdb

Each first-level chapter is a function category; each second-level section a function or operator. Every sample is a real KQL query run against the *typecheck*, *temporal* or *northwind* demo database and transpiled to duckdb SQL.
...
Rendering: `regexp_matches(string, pattern, flags)`
...
Rendering: `regexp_extract(string, pattern)`
...
Rendering: `date_diff('day', start, end)`
...
Rendering: `last_day(date_trunc('quarter', value) + INTERVAL 2 MONTH)`
...
Rendering: `last_day(date_trunc('year', value) + INTERVAL 11 MONTH)`
