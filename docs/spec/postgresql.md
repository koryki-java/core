...
# Function & operator specification: postgresql

Each first-level chapter is a function category; each second-level section a function or operator. Every sample is a real KQL query run against the *typecheck*, *temporal* or *northwind* demo database and transpiled to postgresql SQL.
...
Rendering: `CAST(end::date - start::date AS INTEGER)`
...
-- ignore=sqlite
SELECT
  CAST(DATE '2023-02-01'::date - DATE '2023-03-01'::date AS INTEGER) AS signed_minus28
FROM
 check_temporal c
```
...
Rendering: `CAST(date_trunc('month', value) + INTERVAL '1 month - 1 day' AS DATE)`
...
Rendering: `(CAST(EXTRACT(YEAR FROM age(end, start)) * 12 + EXTRACT(MONTH FROM age(end, start)) AS INTEGER))`
...
Rendering: `CAST(date_trunc('quarter', value) + INTERVAL '3 months - 1 day' AS DATE)`
...
Rendering: `CAST(date_trunc('year', value) + INTERVAL '1 year - 1 day' AS DATE)`
