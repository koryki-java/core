...
# Function & operator specification: oracle

Each first-level chapter is a function category; each second-level section a function or operator. Every sample is a real KQL query run against the *typecheck*, *temporal* or *northwind* demo database and transpiled to oracle SQL.
...
Rendering: `character_length(string)`
...
Rendering: `substr(string, start, length)`
...
Rendering: `EXTRACT(DAY FROM value)`
...
Rendering: `TRUNC(CAST(end AS DATE) - CAST(start AS DATE))`
...
Rendering: `EXTRACT(HOUR FROM value)`
...
Rendering: `EXTRACT(MINUTE FROM value)`
...
Rendering: `EXTRACT(MONTH FROM value)`
...
Rendering: `TRUNC(LAST_DAY(value))`
...
Rendering: `TRUNC(MONTHS_BETWEEN(end, start))`
...
Rendering: `LAST_DAY(ADD_MONTHS(TRUNC(value, 'Q'), 2))`
...
Rendering: `EXTRACT(SECOND FROM value)`
...
Rendering: `EXTRACT(YEAR FROM value)`
...
Rendering: `LAST_DAY(ADD_MONTHS(TRUNC(value, 'YYYY'), 11))`
...
Rendering: `CAST(value AS VARCHAR(4000))`
...
SELECT
  CAST(c.type_integer AS VARCHAR(4000)) AS integer_text
FROM
 check_type c
```
...
Rendering: `TO_TIMESTAMP(value, format)`
...
Rendering: `FROM_TZ(TO_TIMESTAMP(value, format), 'UTC') AT TIME ZONE tz`
...
Rendering: `TO_TIMESTAMP(year || '-' || month || '-' || day || ' ' || hour || ':' || minute || ':' || second, 'YYYY-MM-DD HH24:MI:SS')`
...
Rendering: `FROM_TZ(TO_TIMESTAMP(year || '-' || month || '-' || day || ' ' || hour || ':' || minute || ':' || second, 'YYYY-MM-DD HH24:MI:SS'), 'UTC') AT TIME ZONE tz`
