...
# Function & operator specification: snowflake

Each first-level chapter is a function category; each second-level section a function or operator. Every sample is a real KQL query run against the *typecheck*, *temporal* or *northwind* demo database and transpiled to snowflake SQL.
...
-- ignore=sqlite
SELECT
  DATEDIFF(day, DATE '2023-03-01', DATE '2023-02-01') AS signed_minus28
FROM
 check_temporal c
```
...
Rendering: `LAST_DAY(value, 'quarter')`
