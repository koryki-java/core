...
`BETWEEN` with a **literal** upper bound that names a *day* is not a plain inclusive range — it is
rewritten to a half-open interval so the range covers that whole day. A DATE always names a day; a
TIMESTAMP does so only when its time is the end of the day. See *Unit boundaries: begin / end*
below for the rewrite and its caveats.
...
**A distance counts whole units that fit inside the interval, not boundaries it
crosses.** For two dates the two readings coincide, so this only shows once a time
of day is involved:

    days_between("2023-01-01 23:00:00", "2023-01-02 01:00:00")  =  0

Two hours is not a day, even though a midnight lies between them — the question is
"how many whole days lie between", the same question the table below asks, and it is
the whole-unit truncation of the exact elapsed span that `TIMESTAMP - TIMESTAMP`
gives. Counting the midnight instead would make `days_between` a calendar operation,
and the calendar view belongs to `calendar_distance` and the boundary family.

This was left implicit for a long time, and the engines duly split five to three:
DuckDB, PostgreSQL, SQL Server, Snowflake and SQLite counted the boundary, MariaDB,
Oracle and Trino the elapsed day. All eight now count the elapsed day, matching
`java.time`'s `ChronoUnit`.
...
`BETWEEN` with a literal upper bound that names a **day** is automatically rewritten to the same
half-open form — `expr BETWEEN lower AND upper` becomes `expr >= lower AND expr < (upper + 1d)`.

A `DATE` always names a day. A `TIMESTAMP` names an *instant*, and is only read as a day when its
time is the last one the notation can express — `TIMESTAMP_STRING` is `HH:MI:SS` with optional
milliseconds, so that means `23:59:59.999`, `23:59:59` or `23:59:00`. Any other time is the one the
author chose and is kept, with the inclusive upper bound SQL gives it:

    ... BETWEEN "1996-07-01 00:00:00" AND "1996-12-31 23:59:59"   ->  >= lower AND < 1997-01-01 00:00:00
    ... BETWEEN "1996-07-01 00:00:00" AND "1996-12-31 17:00:00"   ->  BETWEEN lower AND 1996-12-31 17:00:00

Every timestamp upper bound used to be widened to the next midnight, which was right for the first
line and silently added seven hours to the second. `23:59:00` is the one judgement call: it could
equally be that exact instant, but minute precision is not writable, so it is the only way to say
"the last minute" — and a bound meant as an exact instant is far more likely to carry a non-zero
second.

When the upper bound is a function call like `month_end(...)`, no rewrite fires and the half-open
pattern above must be written explicitly.

The rewrite applies wherever the condition stands — `FILTER`, `HAVING`, and inside a projection,
`ORDER` or `GROUP` expression that reaches it through a conditional such as `case(...)`. It used to
visit only filter and having, so the identical range meant one thing in a `FILTER` and another in a
`FETCH`; the range a query names cannot depend on where in the query it is named.
...
A duration is **two quantities, not a list of units**: a calendar amount
(`y q mo`, summed into months) and a day-and-clock amount (`d h m s ms`). The
calendar amount is applied first, in a **single** step, and it is the only one
that can clamp; the day-and-clock amount follows.

The two cannot be merged, because a month has no fixed number of days — this is
the same split `java.time` draws between `Period` and `Duration`. Within the
calendar amount there is nothing to keep apart: a year *is* twelve months, so
`1y1mo` is one step of thirteen months, not a year followed by a month.

    2024-02-29 + 1y1mo  =  2025-03-29      (13 months in one step)

Applying the year and the month separately would clamp on February and land on
2025-03-28 instead. `java.time` agrees: `Period.of(1, 1, 0).toTotalMonths()` is
13, and `LocalDate.of(2024, 2, 29).plus(Period.of(1, 1, 0))` is 2025-03-29.
...
The calendar amount applies **whole**, not unit by unit: `+ 2mo` is one step of
two months, which is why `2023-01-31 + 2mo` lands on March 31 while applying
`+ 1mo` twice would clamp at February and land on March 28. A duration of
`2mo` and a duration of `1mo` applied twice are different operations — and for
the same reason `1y1mo` is thirteen months at once, not two steps.

Clamping never *promotes*: a source day that is its month's last stays on the
same day number when the target month has it. `2022-09-30 + 1mo` is October 30,
not October 31. (Oracle's `ADD_MONTHS` promotes and is corrected for this.)
