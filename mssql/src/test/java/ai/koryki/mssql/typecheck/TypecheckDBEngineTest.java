...
public class TypecheckDBEngineTest extends BaseEngineTest<HeaderInfo> {
    ...
    public void testSingleFile() throws IOException {
        Path kql = queriesRoot().resolve("to_text/numeric.kql");
        // Pass the dialect: the shorter overload leaves it null, and TestUtil then never sees an
        // ignore= marker. This convenience test happens to point at the one fixture that carries
        // one, so it failed where the parameterised path correctly skipped.
        TestUtil.test(kql, suffix(), engine, queriesRoot(), expectedCsv(), expectedSql(), db);
    }
}
