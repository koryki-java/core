...
public class MssqlDialect implements SqlDialect {
     ...
     */
    private static final java.util.Map<String, String> WINDOWS_ZONE =
    ...
    private static String windowsZone(java.time.ZoneId zone) {
        ...
        if (win == null) {
            throw new ai.koryki.antlr.KorykiaiException(
                    "no Windows time-zone name mapped for '"
                            + zone.getId()
                            + "' (SQL Server AT TIME ZONE requires Windows names)");
        }
        return win;
    }

    /**
     * Wall-clock(zone) → model zone: {@code AT TIME ZONE} (Windows names) reads naive as declared,
     * then shifts.
     */
    @Override
    ...
    /**
     * at_zone / to_utc: the zone args arrive SQL-quoted as IANA names; map each to its Windows
     * name.
     */
    /**
    ...
    /**
     * Non-ASCII literals as NVARCHAR ({@code N'...'}): a bare literal materializes in the session
     * database's collation, and converting e.g. Latin1 {@code 'München'} (implicit varchar(7)) into
     * a UTF-8 collated column truncates to {@code 'Münche'}; TRANSLATE similarly mis-counts UTF-8
     * argument lengths. ASCII literals are collation-invariant and stay plain, so transpile-time
     * literal inspection (date_trunc units, to_char masks) keeps working.
     */
    @Override
    ...
    private static FunctionRenderer buildFunctionRenderer() {
        ...
        registry.register(
                new FunctionDefinition(registry.overloads("substring").get(0)) {
                    @Override
                    protected String renderBody(
                        ...
                        String len =
                                args.size() > 2
                                        ? renderer.toSql(args.get(2), indent)
                                        : "LEN(" + s + ")";
        ...
        // complete days inside the interval (24-hour periods), not midnights crossed — measured,
        // the two readings split the engines five to three. DATEDIFF_BIG, not DATEDIFF: a span in
        // seconds overflows a 32-bit count after 68 years.
        registry.override("days_between", "(DATEDIFF_BIG(second, {0}, {1}) / 86400)");
        registry.override(
                "months_between",
                "(DATEDIFF(month, {0}, {1}) - CASE WHEN DAY({1}) < DAY({0}) THEN 1 ELSE 0 END)");
        registry.override("month_end", "EOMONTH({0})");
        registry.override(
                "week_begin", "DATEADD(day, -(DATEDIFF(day, 0, {0}) % 7), CAST({0} AS DATE))");
        registry.override(
                "week_end", "DATEADD(day, 6 - (DATEDIFF(day, 0, {0}) % 7), CAST({0} AS DATE))");
        registry.override(
                "quarter_end", "EOMONTH(DATEADD(QUARTER, DATEDIFF(QUARTER, 0, {0}), 0), 2)");
            ...
            registry.register(
                    new FunctionDefinition(registry.overloads(name).get(0)) {
                        ...
                        protected String renderBody(
                            ...
                            return "TRIM("
                                    + renderer.toSql(args.get(1), indent)
                                    + " FROM "
                                    + s
                                    + ")";
                        }
        ...
        registry.register(
                new FunctionDefinition(registry.overloads("starts_with").get(0)) {
                    @Override
                    protected String renderBody(
                            SqlSelectRenderer renderer, Function function, int indent) {
                        return "CAST(CASE WHEN "
                                + predicateSql(renderer, function, indent)
                                + " THEN 1 ELSE 0 END AS BIT)";
                    }

                    @Override
                    public String renderPredicate(
                            SqlSelectRenderer renderer, Function function, int indent) {
        ...
        registry.override(
                "power",
                "POWER(CAST({0} AS FLOAT), {1})"); // T-SQL POWER keeps the 1st arg's type: DECIMAL
        // input truncates

        // numeric / temporal casts — T-SQL type names ("timestamp" is rowversion; no
        // BOOLEAN/DOUBLE)
        ...
        registry.override(
                "make_timestamp", "DATETIME2FROMPARTS({0}, {1}, {2}, {3}, {4}, {5}, 0, 0)");
        ...
        registry.register(
                new FunctionDefinition("date_trunc", ReturnTypes.ARG1) {
                    ...
                    protected String renderBody(
                        ...
                        return switch (unit) {
                            // DATEDIFF returns int, so the anchor matters: minutes since 1900 is
                            // ~66 M
                            // in 2026 and fits, seconds would not -- which is why there is no
                            // second here.
        ...
        registry.override(
                "split_part", "(SELECT value FROM STRING_SPLIT({0}, {1}, 1) WHERE ordinal = {2})");
        ...
        for (String fn :
                java.util.List.of(
                        ...
                        "initcap",
                        "to_number",
                        // T-SQL PARSE/CONVERT take a culture or a numeric style code, not a format
                        // mask
                        "parse_date",
                        "parse_time",
                        "parse_timestamp")) {
    ...
    /**
     * Compile a literal format mask into a CONCAT of T-SQL date-part expressions. Azure SQL Edge
     * has no {@code FORMAT}, so the mask cannot be handed to the engine as a format string — but
     * the scan itself (longest match over the canonical tokens, quoted literals verbatim) is {@link
     * FormatMask#scan}, shared with every dialect that merely translates.
     */
    ...
    /**
     * SQL Server INSTANT columns are {@code DATETIMEOFFSET}; render the literal with an explicit
     * offset.
     */
    @Override
    public String instantLiteral(java.time.Instant instant, java.time.ZoneId modelZone) {
        return "CAST('"
                + instant.atZone(modelZone).format(DATETIMEOFFSET_FMT)
                + "' AS datetimeoffset)";
    }
    ...
    public String limitClause(int limit, boolean hasOrderBy, int indent) {
        ...
        b.append(Identifier.indent(indent))
                .append("OFFSET 0 ROWS FETCH NEXT ")
                .append(limit)
                .append(" ROWS ONLY")
                .append(SqlRenderer.NL);
        return b.toString();
     ...
     * a nested {@code DATEADD(unit, ±n, base)} (DATEADD clamps month-end like the model and
     * promotes a DATE to DATETIME once a clock unit is added). A DATE_FROM_EPOCH_DAY column becomes
     * its date via {@code DATEADD(DAY, <col>, CAST('1970-01-01' AS DATE))}.
     */
    ...
    /**
     * T-SQL has no {@code EXTRACT(EPOCH ...)}; {@code DATEDIFF_BIG(SECOND, '1970-01-01', ...)}
     * gives epoch-seconds. An INSTANT is a DATETIMEOFFSET — normalize it to a UTC wall-clock
     * DATETIME2 first so the epoch is the true instant regardless of the stored offset.
     */
    @Override
    public String timestampToEpochSeconds(String expr, boolean instant) {
        // SWITCHOFFSET(.., 0) normalizes a DATETIMEOFFSET to UTC (preserving the instant) before
        // the
    ...
    /**
     * T-SQL has no {@code make_timestamp}; {@code DATEADD(SECOND, seconds, '1970-01-01')} →
     * DATETIME (UTC).
     */
    @Override
    ...
    /**
     * T-SQL uses {@code CONCAT} ({@code ||} is not concatenation); it casts the numbers to text
     * implicitly.
     */
    @Override
    ...
    /**
     * SQL Server has no {@code MOD} function (the operator is {@code %}) and no {@code
     * EXTRACT(EPOCH …)}; {@code DATEDIFF} against midnight gives the seconds of a TIME.
     */
    @Override
     ...
     * already come back as a comparison from the default, so only the native case needs the {@code
     * = 1}.
     */
    @Override
