...
/**
 * Physical type-name vocabulary: the dialect type-name strings (the value of a {@link
 * TypeDescriptor#getPhysicalTypeName()}). Extracted from {@link TypeDescriptor} because these names
 * are a separate concern from the descriptor value type — they are the vocabulary consumed by
...
public final class TypeNames {
    ...
    /** PostgreSQL and Trino spell the 8-byte float this way; the SQL standard name. */
    public static final String TYPE_DOUBLE_PRECISION = "DOUBLE PRECISION";

    /** Oracle's 8-byte IEEE float (BINARY_FLOAT is the 4-byte one). */
    public static final String TYPE_BINARY_DOUBLE = "BINARY_DOUBLE";

    /** Oracle's 4-byte IEEE float. */
    ...
    /**
     * MariaDB/MySQL naive timestamp (their TIMESTAMP type is session-zone-converted — see INSTANT
     * encoding).
     */
    ...
    public static final String TYPE_TIMESTAMP_WITH_LOCAL_TIME_ZONE =
