...
/**
 * How a column's values should read — the presentation hint {@code TypeDescriptor} deliberately
 * does not carry.
 *
 * <p>The SQL type cannot answer "how many decimals": measured across the corpus, {@link
 * ai.koryki.catalog.types.TypeDescriptor#getScale() TypeDescriptor.getScale()} is {@code -1} on
 * every column except an explicit {@code to_decimal(x,16,8)}. What can answer it is the semantic
 * layer — a money column shows the currency's minor units, a count shows none — and that knowledge
 * has to reach the read boundary somehow. This is the carrier.
 *
 ...
 * fields cannot express those. The shape mirrors {@link ai.koryki.catalog.types.TypeEncoding
 * TypeEncoding}: a name that serialises to a single string, prefix-parameterised, resolvable
 * through {@link PresentationRegistry} — so a presentation can be written into {@code db.json}
 * exactly the way {@code typeEncoding} already is.
 *
