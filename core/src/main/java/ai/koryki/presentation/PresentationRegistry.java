...
/**
 * Resolves a {@link Presentation} from its serialised name, so a presentation can travel as a
 * single string — in {@code db.json} beside {@code typeEncoding}, or on the wire.
 *
 * <p>Deliberately the same shape as {@link ai.koryki.catalog.types.TypeEncodingRegistry
 * TypeEncodingRegistry}: fixed names in the map, prefix-parameterised ones parsed once and cached.
 * An unknown name is an error rather than a silent null — a typo in a catalog annotation must not
 * read as "no presentation wanted".
 */
public final class PresentationRegistry {
    ...
    /**
     * Prefix-parameterized presentations: {@code DECIMALS:<n>}, {@code SIGNIFICANT:<n>}, {@code
     * SCALE:<factor>:<symbol>}.
     */
