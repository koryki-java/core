...
public class DateBetweenRewriter {
    ...
    private static void rewriteSelect(Select select, List<Runnable> restores) {
        ...
        // The projection, and the ORDER and GROUP expressions, were not visited at all. A BETWEEN
        // reaches them through a conditional — `case(o.ts BETWEEN a AND b, …)` — and it then meant
        // something different from the identical condition in a FILTER: inclusive there, half-open
        // here. The range a query names cannot depend on where in the query it is named.
        select.getOut().forEach(o -> rewriteExpression(o.getExpression(), restores));
        select.getOrder().forEach(o -> rewriteExpression(o.getExpression(), restores));
        select.getGroup().forEach(g -> rewriteExpression(g.getExpression(), restores));
    ...
    /**
     * Whether a TIMESTAMP bound names the <strong>last instant of its day that the notation can
     * express</strong>. {@code TIMESTAMP_STRING} is {@code HH:MI:SS} with optional milliseconds, so
     * there are exactly three such spellings:
     *
     * <pre>
     *   23:59:59.999   the last millisecond
     *   23:59:59       the last second
     *   23:59:00       the last minute — minute precision is not writable, so this is how it is said
     * </pre>
     *
     * Anything else is a time the author chose deliberately and is left alone: {@code 23:59:58} is
     * not end-of-day, and neither is {@code 17:00:00}.
     *
