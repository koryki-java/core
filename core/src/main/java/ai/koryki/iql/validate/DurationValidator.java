...
/**
 * A DURATION literal must list its components largest-unit-first ({@code 1y2mo15d4h}, not {@code
 * 4h15d2mo1y}). The grammar accepts any order; this turns an out-of-order literal into a positioned
 * validation error instead of silently reordering it at application time, so the written order
 * matches the order in which the components are applied.
 */
public class DurationValidator implements Visitor, Collector<List<Violation>> {
    ...
    public boolean visit(Deque<Object> deque, Expression expression) {
        ...
        if (duration != null && !largestFirst(duration.getComponents())) {
            String canonical =
                    duration.getComponents().stream()
                            .sorted(
                                    Comparator.comparingInt(
                                                    (Duration.Component c) -> c.unit().ordinal())
                                            .reversed())
            ...
            violations.add(
                    new Violation(
                            ...
                            "duration '"
                                    + duration
                                    + "' must list components largest-unit-first — write '"
                                    + canonical
                                    + "'"));
        }
        return true;
    ...
    /**
     * Largest-first means no component names a larger unit than the one before it (Unit ordinals
     * ascend by size).
     */
