...
public class SchemaValidator implements Collector<List<Violation>> {
     ...
     * fields are visited in document order, so a field was resolved before the sources written
     * after it had even been recorded. {@code block_join.iql} is both at once — {@code o} names
     * {@code orders} inside the WITH block and the block {@code ord} outside it, and the outer
     * {@code o.f} is written one line before the {@code JOIN} that binds it. It resolved to {@code
     * orders}.
     *
     ...
     *
     * <p>Reported here rather than where the join is resolved, for two reasons. A resolver throws
     * on the first bad column and stops, so a query with several mistakes reveals them one run at a
     ...
     * first join, and each join's own source is the left of the joins nested under it. A {@code
     * Join} does not carry its left side, so it cannot be checked in isolation.
     *
    ...
    private void checkJoinSide(Object anchor, Source source, java.util.List<String> columns) {
        Optional<ai.koryki.catalog.domain.Entity> entity =
                resolver.getModel().getEntity(source.getName());
        if (entity.isEmpty()) {
            return;
        }
    ...
    /**
     * A block bound to a placeholder has no set — same reason as in {@code
     * BlockLeadingSourceCollector}.
     */
    @Override
    public boolean visit(Deque<Object> deque, Block block) {
        ...
        blockIdToSelectMap.put(
                block.getId(), SelectScopeCollector.getLeadingSelect(block.getSet()));
    ...
    public boolean visit(Deque<Object> deque, Source source) {
        ...
        if (recursiveAliasToSourceMap != null
                && recursiveAliasToSourceMap.containsKey(source.getName())) {
                ...
                if (outs.stream().anyMatch(o -> match(field, o))) {
                    ...
                    violations.add(
                            new Violation(
                                    ...
                                    .suggesting(
                                            Suggest.closest(field.getName(), headerNames(outs))));
                }
            } else {
                Optional<ai.koryki.catalog.domain.Entity> optional =
                        resolver.getModel().getEntity(selectTable.getName());
                if (optional.isEmpty()) {
                    violations.add(
                            new Violation(
                                            "schema",
                                    ...
                                    .suggesting(
                                            Suggest.closest(selectTable.getName(), entityNames())));
                } else {
     ...
     * column the entity does not have passed validation untouched and blew up later in {@code
     * ExpressionTypeResolver.resolveField}, on a bare {@code Optional.get()}, as {@code
     * NoSuchElementException: No value present} — no position, no column name, nothing to act on.
     *
     ...
     * vocabulary arriving by a side door — under the German model it would accept {@code
     * o.order_id} for {@code bestell_id}. {@link #attribute} does not.
     *
    ...
    private static boolean match(Field column, Out o) {
        return column.getName().equals(o.getHeader())
                || (o.getExpression().getField() != null
                        && column.getName().equals(o.getExpression().getField().getName()));
    }
    ...
    private static List<String> attributeNames(ai.koryki.catalog.domain.Entity entity) {
        return entity.getAttributes().stream()
                .map(ai.koryki.catalog.domain.Attribute::getName)
                .toList();
    }
     ...
     */
    private static List<String> columnSuggestions(
            ai.koryki.catalog.domain.Entity entity, String name) {
    ...
    private List<String> entityNames() {
        return resolver.getModel().getEntities().stream()
                .map(ai.koryki.catalog.domain.Entity::getName)
                .toList();
    }
