...
/**
 * Rendering support for column-level INTERVAL encodings — the comparison/arithmetic dual of the
 * read-side {@link ai.koryki.jdbc.CoreDecoder}. When a duration literal is compared with an
 * INTERVAL column, it must be rendered in the column's own storage encoding so both operands share
 * one physical representation:
 *
 * <ul>
 *   <li>{@code INTERVAL:<unit>} (numeric count) → an integer count of the unit ({@code 1h2min3s}
 *       against a SECONDS column → {@code 3723});
 *   <li>{@code INTERVAL_CHAR} / {@code INTERVAL_FROM_STRING} (text) → an ISO-8601 string literal
 *       ({@code 'PT1H2M3S'});
 *   <li>native INTERVAL (no encoding) → empty: the caller renders the duration as a native interval
 *       value.
 * </ul>
 *
 * Like {@link TimeEncodings}, the encoding is a property of the column, not the dialect; dialects
 * opt in by consulting this helper in their comparison hook.
 */
public final class IntervalEncodings {
    ...
    /**
     * Renders {@code operand} (a duration literal) in {@code columnType}'s INTERVAL storage
     * encoding, or empty when it does not apply (not an INTERVAL column, not a duration literal, or
     * a native interval the caller renders itself).
     */
    ...
    /**
     * A whole count of {@code unit} for an interval expressible in that unit (else throws). The
     * three fields never cross: month-family units read {@code months}, day-family units read
     * {@code days}, fixed clock units read {@code nanos}. A day is a variable (calendar) unit — it
     * is <em>not</em> 86_400s — so it never reduces to or from a clock unit (docs/TEMPORAL.md,
    ...
    static long inUnit(Interval iv, ChronoUnit unit) {
        ...
        if (unit == ChronoUnit.WEEKS || unit == ChronoUnit.DAYS) {
            ...
            if (unit == ChronoUnit.WEEKS) {
                if (iv.getDays() % 7 != 0) {
                    throw new UnsupportedOperationException(
                            "duration is not a whole number of weeks");
                }
                return iv.getDays() / 7;
            }
            return iv.getDays();
        ...
        if (iv.getMonths() != 0 || iv.getDays() != 0) {
            throw new UnsupportedOperationException(
                    "an INTERVAL:"
                            + unit
                            + " column cannot hold a calendar (day/month/year) component");
        }
