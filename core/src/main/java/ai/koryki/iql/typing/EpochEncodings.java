 ...
 * against a date/timestamp literal. An {@code EPOCH:<unit>} or {@code DATE_FROM_EPOCH_DAY} column
 * is physically an integer, so the literal is rendered as the matching integer count — keeping the
 * column bare and the filter index-friendly (the same approach as {@link
 * IntervalEncodings#durationOperand}).
 *
 * <p>The literal is a model-zone wall-clock value, so an EPOCH count is taken by interpreting it in
 * the {@code modelZone} threaded from the renderer (docs/TEMPORAL.md: comparisons convert the
 * literal, not the column). DATE_FROM_EPOCH_DAY is a zone-free day count. A native / zone-aware
 * (INSTANT) column is left to the dialect's own literal coercion and is not reconciled here.
 */
public final class EpochEncodings {

    private EpochEncodings() {}

    public static Optional<String> literalOperand(
        ...
        if (enc instanceof EpochTypeEncoding epoch) {
            Instant instant =
                    (dateTime != null ? dateTime : date.atStartOfDay())
                            .atZone(modelZone)
                            .toInstant();
