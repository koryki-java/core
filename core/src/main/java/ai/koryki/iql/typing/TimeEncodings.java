...
/**
 * Rendering support for column-level TIME encodings. The encoding is a property of the column
 * (declared in the schema's type descriptors), not of the dialect — any dialect whose schemas may
 * carry such columns opts in by consulting this helper in its rendering hooks.
 */
public final class TimeEncodings {
    ...
    /**
     * If the compared column stores TIME as seconds from midnight and the operand is a time
     * literal, the literal renders as its second-of-day value (e.g. {@code TIME '14:30:00'} →
     * {@code 52200}).
     *
     * @return the seconds literal, or empty if the encoding does not apply
     */
    public static Optional<String> secondsFromMidnightLiteral(
            TypeDescriptor columnType, Expression operand) {
        if (columnType != null
    ...
    // Cross-encoding comparison of two columns is no longer a TIME-specific question: the policy
    // ("which side converts") is EncodingLattice's, applied for every family by
    // SqlDialect.renderComparison. What is still missing for TIME is the conversion itself —
    // TIME_FROM_STRING "12:30:50" <-> TIME_FROM_INTEGER 123050 — which belongs in
    // EncodingLattice.losslessTargets/convertSql, not in a dialect hook. Until it lands there,
    // such a comparison is reported as a warning by FunctionValidator rather than mis-rendered.

    /**
     * Anchor day for encodings that embed a time of day in a DATE/TIMESTAMP column — must match the
     * {@code DATE '1970-01-01'} anchor used when rendering such columns for output.
     */
    public static final LocalDate EPOCH_DAY = LocalDate.of(1970, 1, 1);

    /**
     * If the compared column stores TIME truncated from a DATE ({@link
     * CoreTypeEncoding#TIME_FROM_DATE}) and the operand is a time literal, the operand becomes a
     * date-time on the anchor day (e.g. {@code TIME '14:30:00'} → {@code 1970-01-01T14:30:00}). The
     * literal syntax for that value is the dialect's decision (e.g. {@code TO_DATE(...)} on Oracle,
     * {@code TIMESTAMP '...'} ANSI).
     *
     * @return the epoch-anchored date-time, or empty if the encoding does not apply
     */
    public static Optional<LocalDateTime> timeFromDateOperand(
            TypeDescriptor columnType, Expression operand) {
        if (columnType != null
     ...
     * seconds-from-midnight (already seconds), HHMMSS integer, or 'HH:MM:SS' text. A
     * <em>native</em> TIME column (no encoding) is NOT included — it keeps the dialect-native
     * {@code TIME + INTERVAL} path. Both the result-type ({@code ReturnTypes.ADD_SUB}) and the
     * rendering ({@link #secondsArithmetic}) key off this single predicate so they always agree.
     */
    ...
    public static boolean isDurationOperand(Expression right) {
        return right.getDuration() != null
                || (right.getFunction() != null
                        && "to_interval".equals(right.getFunction().getFunc()));
    }

    /**
     * Wraps a seconds-of-day arithmetic result back into [0, 86400) so that crossing midnight in
     * either direction stays a valid time-of-day (23:30 + 2h → 01:30, 00:30 - 1h → 23:30). MOD
     * keeps the dividend's sign in every supported dialect, so the second MOD after adding 86400
     * folds a negative (pre-midnight) result back into range. Requires a MOD(a, b) function
     * (Oracle, PostgreSQL, DuckDB, Snowflake).
     */
    ...
    /**
     * TIME ± duration on a seconds-from-midnight column: the operand becomes integer seconds added
     * to/subtracted from the raw column. The result is emitted as plain integer arithmetic —
     * wrapping into [0, 86400) ("wraps around midnight", docs/TEMPORAL.md) happens at the read
     * boundary ({@code JdbcDatabase#read} decodes the integer to a LocalTime with floorMod), so the
     * SQL needs no MOD and works on every dialect.
     *
     * @return the raw seconds expression, or empty if the encoding/operand shape does not apply
     */
    public static Optional<String> secondsArithmetic(
        ...
        return Optional.of(
                leftSeconds + " " + operator + " " + durationSeconds(renderer, right, indent));
    }

    /**
     * Renders a duration / to_interval operand as a total-seconds expression, for adding to or
     * subtracting from a seconds-from-midnight TIME. Calendar units (YEAR/MONTH/QUARTAL) and
     * sub-second precision are rejected — neither survives an integer seconds-of-day encoding.
     */
    public static String durationSeconds(SqlSelectRenderer renderer, Expression right, int indent) {
        ...
        if (f.getArguments().size() == 6) {
            ...
            // parenthesise the sum so a leading '-' operator negates the whole delta, not just term
            // 0
    ...
    private static long componentSeconds(Duration.Component c) {
        return switch (c.unit()) {
            ...
            // rejected on a time-of-day too (FunctionValidator rejects this earlier with a
            // position).
            case DAY, WEEK, YEAR, MONTH, QUARTAL ->
                    throw new UnsupportedOperationException(
                            "cannot add "
                                    + c.unit()
                                    + " to a TIME value — only fixed clock units (h m s) apply to a time-of-day.");
        };
    }
    ...
    private static String intervalUnitSeconds(String value, String unit) {
        ...
        if (bare == null) {
            throw new UnsupportedOperationException(
                    "to_interval unit must be a literal for TIME arithmetic");
        }
        return switch (bare) {
