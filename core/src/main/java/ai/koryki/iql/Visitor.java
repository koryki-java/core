...
public interface Visitor {
    ...
    static Source findSourceInSelect(Select select, String alias) {
        return findInFromTree(select.getStart(), select.getJoin(), alias);
    }

    static Source findSourceInExists(Exists exists, String alias) {
        return findInFromTree(exists.getStart(), exists.getJoin(), alias);
    }
    ...
    private static Source findInFromTree(Source start, List<Join> joins, String alias) {
        ...
        for (Join j : joins) {
            Source found = findInFromTree(j.getSource(), j.getJoin(), alias);
            if (found != null) {
                return found;
            }
        }
