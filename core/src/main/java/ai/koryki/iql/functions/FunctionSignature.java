...
public record FunctionSignature(List<FunctionArg> args, boolean variadic) {
    ...
    public String toString() {
        String s = render(args, FunctionArg::name);
        if (variadic) {
            s = s + ", ...";
        }
        return "(" + s + ")";
    ...
    /**
     * Joins the arguments, bracketing the optional ones — {@code string, start [, length]}.
     *
    ...
    public static String render(List<FunctionArg> args, java.util.function.Function<FunctionArg, String> token) {
        ...
        for (int i = 0; i < args.size(); i++) {
            ...
            if (a.optional()) {
                b.append(i == 0 ? "[" : " [, ").append(token.apply(a)).append(']');
            } else {
                if (i > 0) {
                    b.append(", ");
                }
                b.append(token.apply(a));
            }
        }
        return b.toString();
    }
