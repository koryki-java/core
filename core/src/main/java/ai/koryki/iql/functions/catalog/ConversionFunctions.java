...
/**
 * Type-cast functions: one name per target type ({@code to_date}, {@code to_integer}, …) rather
 * than a single {@code cast(value, type)}. The type is part of the name because it is not a value —
 * it cannot come from a column or a parameter — and naming it makes every call resolvable by the
 ...
 *
 * <p>{@code to_text} is the exception in shape: it is registered once per source type-family, so
 * the documentation can say what is being converted rather than listing one entry that accepts
 * anything.
 *
 ...
 * and SQL Server's bare {@code CAST} truncates, so both wrap the value in {@code ROUND} (see {@code
 * SqliteDialect}, {@code MssqlDialect}). Without it the same query answered differently depending
 * on the database, and both answers looked plausible.
 */
public final class ConversionFunctions {
    ...
    public static void register(FunctionRegistry r) {
        r.register(
                cast("to_date", ReturnTypes.DATE, "DATE", Families.TEMPORAL_OR_TEXT)
                        .doc(
                                "Converts a timestamp or date-compatible value to a DATE, discarding any time "
        ...
        r.register(
                cast("to_time", ReturnTypes.TIME, "TIME", Families.TEMPORAL_OR_TEXT)
                        .doc(
                                "Converts a timestamp or time-compatible value to a TIME, discarding the date part. "
                                        + "Text input must be in `HH:MM:SS` format."));
        r.register(
                cast("to_timestamp", ReturnTypes.TIMESTAMP, "TIMESTAMP", Families.TEMPORAL_OR_TEXT)
                        .doc(
                                "Converts a date or text value to a TIMESTAMP. Text input must be ISO 8601 "
                                        + "(`YYYY-MM-DD HH:MM:SS`); a DATE is extended with midnight."));
        r.register(
                cast("to_boolean", ReturnTypes.BOOLEAN, "BOOLEAN", Families.NUMBER_OR_TEXT)
                        .doc(
                                "Converts a numeric value to BOOLEAN: `0` is false, anything else is true.")
                        .paragraph(
                                "**What comes back is not spelled the same everywhere.** Databases with a "
        ...
        registerToText(r);
        r.register(
                cast("to_float", ReturnTypes.FLOAT, "FLOAT", Families.NUMBER_OR_TEXT)
                        .doc(
                                "Converts a numeric or text value to single-precision float. Fractional precision "
                                        + "may be lost relative to the source."));
        r.register(
                cast("to_double", ReturnTypes.DOUBLE, "DOUBLE", Families.NUMBER_OR_TEXT)
                        .doc("Converts a numeric or text value to double-precision float."));
        r.register(
                cast("to_integer", ReturnTypes.INTEGER, "INTEGER", Families.NUMBER_OR_TEXT)
                        .doc(
                                "Converts a numeric or text value to a 32-bit integer. A fractional part is "
                                        ...
                                        + "dialect-defined."));
        r.register(
                cast("to_bigint", ReturnTypes.BIGINT, "BIGINT", Families.NUMBER_OR_TEXT)
                        .doc(
                                "Converts a numeric or text value to a 64-bit integer. Rounds like `to_integer`; use "
                                        + "it instead when values may exceed ±2 147 483 647."));
        r.register(
                cast("to_smallint", ReturnTypes.SMALLINT, "SMALLINT", Families.NUMBER_OR_TEXT)
                        .doc(
                                "Converts a numeric or text value to a 16-bit integer (range −32 768 to 32 767). "
                                        + "Rounds like `to_integer`. Overflow behaviour is dialect-defined."));

        r.register(
                new FunctionDefinition("to_decimal", ReturnTypes.DECIMAL_CAST)
                        .category(FunctionCategory.CONVERSION)
                        .args(
                                arg(
                                        "value",
                                        Families.NUMBER_OR_TEXT,
                                        "the value to convert to a decimal"),
                                arg(
                                        "precision",
                                        CoreTypeFamily.INTEGER,
                        ...
                        .doc(
                                "Converts *value* to a fixed-point decimal. *precision* is the total number of "
                                        + "significant digits, *scale* the digits after the point — "
                                        + "`to_decimal(value, 10, 2)` holds up to 99 999 999.99."));
        r.register(
                new FunctionDefinition("to_varchar", ReturnTypes.TEXT)
                        .category(FunctionCategory.CONVERSION)
                        .args(
                        ...
                        .doc(
                                "Converts *value* to a variable-length string with an explicit maximum length — "
     ...
     * overload per source family. They all render identically ({@code CAST(value AS TEXT)}); a
     * dialect needing a different physical type re-renders them all via {@link
     * FunctionRegistry#overrideAll} (see {@code OracleDialect}). All render identically; the
     * BOOLEAN overload additionally tags its TEXT result with TEXT_FROM_BOOLEAN so the read layer
     * can present 0/1 as true/false.
     */
    private static void registerToText(FunctionRegistry r) {
        for (CoreTypeFamily family : CoreTypeFamily.values()) {
            ...
            ReturnTypeInference returns =
                    family == CoreTypeFamily.BOOLEAN
                            ? ReturnTypes.TEXT_FROM_BOOLEAN
                            : ReturnTypes.TEXT;
            ...
            r.register(
                    new FunctionDefinition("to_text", returns)
                            .category(FunctionCategory.CONVERSION)
    ...
    /**
     * Plain {@code CAST(value AS type)} mapping; reused by dialects with their own physical type
     * names.
     */
    public static FunctionDefinition cast(
            String name, ReturnTypeInference type, String sqlType, TypeFamily input) {
