...
/**
 * Mathematical functions, mirroring the PostgreSQL "Mathematical Functions" chapter. Deliberately
 * functions only — PostgreSQL's operator spellings ({@code %}, {@code |/}, {@code ||/}, {@code ^})
 * are not exposed in KQL: the audience is business analysts, and Excel-style {@code mod}/{@code
 * sqrt}/ {@code power} read better than operator symbols ({@code %} in particular reads as
 * "percent").
 */
public final class MathFunctions {

    /**
     * The one point where {@code greatest}/{@code least} are not portable — measured, not copied. A
     * constant, so both functions carry the same wording and it is not updated in one place and
    ...
    public static void register(FunctionRegistry r) {
        r.register(
                def("abs", ReturnTypes.ARG0)
                        ...
                        .doc("Absolute value."));
        r.register(
                def("ceil", ReturnTypes.ARG0)
                        .args(arg("value", Families.NUMERIC, "the number to round up"))
                        .doc("Nearest integer greater than or equal to *value*."));
        r.register(
                def("floor", ReturnTypes.ARG0)
                        .args(arg("value", Families.NUMERIC, "the number to round down"))
                        .doc("Nearest integer less than or equal to *value*."));
        r.register(
                def("round", ReturnTypes.ARG0)
                        .args(arg("value", Families.NUMERIC, "the number to round"))
        ...
        r.register(
                def("round", ReturnTypes.ARG0)
                        .args(
                                arg("value", Families.NUMERIC, "the number to round"),
                                arg(
                                        "scale",
                        ...
                        .paragraph(
                                "Half-way rounding of binary-float inputs is engine-defined: a value such as "
        ...
        r.register(
                def("trunc", ReturnTypes.ARG0)
                        .args(arg("value", Families.NUMERIC, "the number to truncate"))
                        .doc(
                                "Truncates toward zero — 1.99 becomes 1, and -1.99 becomes -1. Unlike `round` "
                                        + "it never carries, so use it where a value must not be inflated and `round` "
                                        + "where it should be nearest."));
        r.register(
                def("trunc", ReturnTypes.ARG0)
                        .args(
                                arg("value", Families.NUMERIC, "the number to truncate"),
                                arg(
                                        "scale",
                                        CoreTypeFamily.INTEGER,
                                        "number of decimal places to keep"))
                        .doc("Truncates toward zero, keeping *scale* decimal places."));
        // The NULLIF mirrors the divide-by-zero guard on `/` (see
        // Families/SqlDialect.renderDivision).
        // Measured, a bare mod(7, 0) errors on postgresql/mssql/trino, is NULL on
        // mariadb/sqlite/duckdb
        ...
        r.register(
                def("mod", ReturnTypes.ARG0)
                        .args(
                                arg("dividend", Families.NUMERIC, "the number being divided"),
                                arg("divisor", Families.NUMERIC, "the number to divide by"))
                        .template("mod({0}, NULLIF({1}, 0))")
                        .doc(
                                "Remainder of *dividend* / *divisor*. A zero *divisor* yields no value rather "
                                        + "than an error, as with `/`."));
        r.register(
                def("sign", ReturnTypes.INTEGER)
        ...
        r.register(
                def("greatest", ReturnTypes.LEAST_RESTRICTIVE)
                        // At least TWO values. With one argument the question is meaningless, and
                        // it was
                        // answered differently: MariaDB rejected it, SQLite read max(x)/min(x) as
                        // an
                        // aggregate and silently folded every row into one.
                        .variadic(
                                arg("value", Families.ORDERED, "the values to compare"),
                                arg("more", Families.ORDERED, "further values to compare against"))
                        .doc(
                                "Largest of the arguments, compared across the columns of one row — not to be "
                                        + "confused with `max`, which collapses rows into one.")
                        .paragraph(NULL_HANDLING_DIFFERS));
        r.register(
                def("least", ReturnTypes.LEAST_RESTRICTIVE)
                        // At least TWO values. With one argument the question is meaningless, and
                        // it was
                        // answered differently: MariaDB rejected it, SQLite read max(x)/min(x) as
                        // an
                        ...
                        .variadic(
                                ...
                                arg("more", Families.ORDERED, "further values to compare against"))
                        .doc(
                                "Smallest of the arguments, compared across the columns of one row — the mirror "
                                        + "of `greatest`, and not to be confused with `min`.")
                        .paragraph(NULL_HANDLING_DIFFERS));

        r.register(
                def("power", ReturnTypes.FLOAT)
                        .args(
                                arg("base", Families.NUMERIC, "the number to raise"),
                                arg("exponent", Families.NUMERIC, "the power to raise the base to"))
                        .doc("*base* raised to the power of *exponent*."));
        r.register(
                def("sqrt", ReturnTypes.FLOAT)
                        .args(
                                arg(
                                        "value",
                                        Families.NUMERIC,
        ...
        // a name that avoids it, so the two readings are spelled out and the bare name is not
        // taken.
        r.register(
                def("ln", ReturnTypes.FLOAT)
                        .args(
                                arg(
                                        "value",
                                        Families.NUMERIC,
        ...
        r.register(
                def("log10", ReturnTypes.FLOAT)
                        .args(
                                arg(
                                        "value",
                                        Families.NUMERIC,
                                        "the number to take the base-10 logarithm of"))
                        .doc("Base-10 logarithm."));
        r.register(
                def("log", ReturnTypes.FLOAT)
                        .args(
                                arg("base", Families.NUMERIC, "the base of the logarithm"),
                                arg(
                                        "value",
                                        Families.NUMERIC,
                                        "the number to take the logarithm of"))
                        .doc(
                                "Logarithm of *value* to the given *base*: log(2, 8) is 3. The base comes first, "
                                        + "as it reads aloud."));

        // Trigonometry, in radians. Included for completeness of the common set; the
        // inverse-hyperbolic
        // and cot/atan2/degrees/radians spellings are not, being both less portable and outside
        // what
        ...
        for (String name : List.of("sin", "cos", "tan", "asin", "acos", "atan")) {
            r.register(
                    def(name, ReturnTypes.FLOAT)
                            .args(arg("value", Families.NUMERIC, "the angle in radians"))
                            .doc("Trigonometric " + name + " (argument in radians)."));
        }

        r.register(def("pi", ReturnTypes.FLOAT).args().doc("Approximate value of π."));
        r.register(
                def("random", ReturnTypes.FLOAT)
                        .args()
