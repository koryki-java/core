...
/**
 * Regular-expression functions, mirroring the PostgreSQL "Pattern Matching" chapter. Functions only
 * — LIKE/SIMILAR TO operator forms are comparison syntax, not catalog functions. Pattern syntax is
 * passed through to the dialect and not translated.
 */
public final class PatternMatchingFunctions {
    ...
    public static void register(FunctionRegistry r) {
        r.register(
                def("regexp_like", ReturnTypes.BOOLEAN)
                        .args(
                                arg("string", TEXT, "the source text to test"),
                                arg("pattern", TEXT, "the regular expression to match against"),
                                optionalArg(
                                        "flags",
                                        TEXT,
                                        "match modifiers, e.g. 'i' for case-insensitivity"))
                        .doc("True if the string matches the regular expression *pattern*."));
        r.register(
                def("regexp_count", ReturnTypes.INTEGER)
                        .args(
                                arg("string", TEXT, "the source text to search"),
                                arg(
                                        "pattern",
                                        TEXT,
                                        "the regular expression to count occurrences of"),
                                optionalArg("start", INT, "1-based index to begin searching from"))
                        .doc("Number of matches of *pattern* in the string."));
        r.register(
                def("regexp_substr", ReturnTypes.TEXT)
                        .args(
                                arg("string", TEXT, "the source text to search"),
        ...
        r.register(
                def("regexp_replace", ReturnTypes.TEXT)
                        .args(
                                arg("string", TEXT, "the source text to modify"),
                                arg("pattern", TEXT, "the regular expression to match"),
                                arg("replacement", TEXT, "the text substituted for each match"))
                        .doc(
                                "Replaces **every** substring matching the regular expression *pattern* with "
                                        + "*replacement*.")
                        .paragraph(
                                "Every match, not just the first — on every database. That guarantee costs "
