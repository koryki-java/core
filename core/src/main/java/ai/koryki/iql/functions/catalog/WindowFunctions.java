 ...
 * collapsing rows, so they must never trigger GROUP BY inference. {@code FunctionValidator}
...
public final class WindowFunctions {
    ...
    public static void register(FunctionRegistry r) {
        r.register(
                def("row_number", ReturnTypes.BIGINT)
                        .signature(FunctionSignature.of())
                        .doc(
                                "Position of the row within its window, counting from 1. Ties are numbered "
                                        + "arbitrarily — use `rank` if equal values should share a number."));
        r.register(
                def("rank", ReturnTypes.BIGINT)
                        .signature(FunctionSignature.of())
                        .doc(
                                "Position of the row by the window's ordering, where equal values share a rank "
                ...
                def("ntile", ReturnTypes.BIGINT)
                        .args(
                                arg(
                                        "buckets",
                                        Families.NUMERIC,
                                        "how many groups to divide the rows into"))
                        .doc(
                                "Splits the window's rows into *buckets* groups of near-equal size and returns "
                                        + "which group the row falls in — quartiles with `ntile(4)`."));
        r.register(
                def("lag", ReturnTypes.ARG0)
                        .signature(
                                FunctionSignature.of(
                                        arg(
                                                "value",
                                                Families.ANY,
                                                "the value to read from an earlier row"),
                        ...
                        .doc(
                                "The value from an earlier row of the window — the previous month's figure, "
                                        + "say, for comparing against this one."));
        r.register(
                def("lead", ReturnTypes.ARG0)
                        .signature(
                                FunctionSignature.of(
                                        arg(
                                                "value",
                                                Families.ANY,
                                                "the value to read from a later row"),
                                        FunctionArg.optionalArg(
                                                "offset",
                                                Families.NUMERIC,
