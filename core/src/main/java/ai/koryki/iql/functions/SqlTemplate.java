...
public final class SqlTemplate {
    ...
    public String render(SqlSelectRenderer renderer, Function function, int indent) {
        ...
        requireNoSurplus(args.size(), function.getFunc());
    ...
    /**
     * The counterpart to the too-few check above. Until now this direction was unguarded: an
     * operand the template never references was simply not rendered, and the caller had no way to
     * notice. That is how {@code a > 1 AND b} could lose its second predicate — the parser handed
     * {@code >} three operands, {@code "{0} > {1}"} used two, and the third disappeared into a
     * silently wrong query.
     *
     * <p>A missing operand was always an error; a surplus one has to be as well. Templates with a
     * variadic tail ({@code {n*}}) are exempt — there the tail is meant to absorb what is left.
     */
    private void requireNoSurplus(int provided, String function) {
        int referenced = 0;
        for (Segment s : segments) {
            if (s instanceof Rest) {
                return;
            }
            if (s instanceof Arg a) {
                referenced = Math.max(referenced, a.index() + 1);
            }
        }
