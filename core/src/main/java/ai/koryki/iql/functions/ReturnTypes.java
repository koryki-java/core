...
public class ReturnTypes {
    ...
    /**
     * calendar_distance: an INTERVAL whose "startEpoch;endEpoch" wire form is decoded to a calendar
     * span.
     */
    public static final ReturnTypeInference CALENDAR_DISTANCE =
            b ->
                    new TypeDescriptor(
                            TypeNames.TYPE_INTERVAL,
                            CoreTypeEncoding.CALENDAR_DISTANCE,
    ...
    /**
     * TIMESTAMP − TIMESTAMP lands here: an exact elapsed span counted in seconds (no calendar
     * fields).
     */
    private static final TypeDescriptor INTERVAL_SECONDS =
            new TypeDescriptor(
                    TypeNames.TYPE_INTERVAL,
                    new IntervalTypeEncoding(ChronoUnit.SECONDS),
                    CoreTypeFamily.INTERVAL);

    /**
     * DATE − DATE lands here: a whole number of days (docs/TEMPORAL.md, "differences come out in
     * whole days"). Counted in DAYS for the same reason its timestamp sibling is counted in SECONDS
     * — every dialect renders the difference as a plain integer, so the encoding has to say what
     * that integer counts. Left as a bare INTERVAL it decoded to nothing at all.
     */
    private static final TypeDescriptor INTERVAL_DAYS =
            new TypeDescriptor(
                    TypeNames.TYPE_INTERVAL,
                    new IntervalTypeEncoding(ChronoUnit.DAYS),
                    CoreTypeFamily.INTERVAL);

    /**
     * Conditional result: reconcile every operand to one common type (coalesce). See {@link
     * ConditionalReconciler}.
     */
    public static final ReturnTypeInference RECONCILE =
            binding -> {
    ...
    public static final ReturnTypeInference LEAST_RESTRICTIVE =
            binding -> {
    ...
    // --- DECIMAL precision/scale propagation (T-SQL-style formulas; clamped per dialect in
    // renderType) ---
    // Each folds left across operands and falls back to LEAST_RESTRICTIVE unless every operand is a
    // DECIMAL
    // carrying precision and scale, so integer/float arithmetic is unaffected.

    /** DECIMAL(p1,s1) * DECIMAL(p2,s2) -> DECIMAL(p1+p2+1, s1+s2). */
    public static final ReturnTypeInference DECIMAL_MULTIPLY =
            decimalFold((a, b) -> new int[] {a[0] + b[0] + 1, a[1] + b[1]});
     ...
     */
    public static final ReturnTypeInference MULTIPLY =
            binding -> {
                if (binding.getOperandCount() >= 2) {
                    TypeFamily a = family(binding.getOperandType(0));
                    TypeFamily b = family(binding.getOperandType(1));
                    boolean aIv = CoreTypeFamily.INTERVAL.equals(a),
                            bIv = CoreTypeFamily.INTERVAL.equals(b);
                    boolean aInt = CoreTypeFamily.INTEGER.equals(a),
                            bInt = CoreTypeFamily.INTEGER.equals(b);
     ...
     */
    public static final ReturnTypeInference DECIMAL_ADD =
            decimalFold(
                    (a, b) -> {
    ...
    /**
     * DECIMAL division (approximate; guarantees >= 6 fractional digits). Clamped per dialect in
     * renderType.
     */
    public static final ReturnTypeInference DECIMAL_DIVIDE =
            decimalFold(
                    (a, b) -> {
     ...
     */
    public static final ReturnTypeInference DECIMAL_CAST =
            binding -> {
                var args = binding.getFunction().getArguments();
                if (args.size() >= 3
                        && args.get(1).getNumber() != null
                        && args.get(2).getNumber() != null) {
                    return new TypeDescriptor(
                            TypeNames.TYPE_DECIMAL,
                            null,
                            CoreTypeFamily.DECIMAL,
                            args.get(1).getNumber().intValue(),
                            args.get(2).getNumber().intValue());
                }
    ...
    private static ReturnTypeInference decimalFold(BinaryOperator<int[]> combine) {
        return binding -> {
            ...
            return new TypeDescriptor(
                    TypeNames.TYPE_DECIMAL, null, CoreTypeFamily.DECIMAL, ps[0], ps[1]);
        };
    }
    ...
    /**
     * Temporal {@code +}/{@code -} per docs/TEMPORAL.md "Result-type algebra"; falls back to {@link
     * #DECIMAL_ADD} for numeric operands. The result <em>strips the storage encoding</em> — a
     * computed DATE is a plain DATE, not the epoch-day integer (etc.) the source column was — and a
     * DATE plus a duration that has any clock component becomes a TIMESTAMP.
     */
    public static final ReturnTypeInference ADD_SUB =
            binding -> {
                if (binding.getOperandCount() >= 2) {
                    ...
                    if (rightInterval && CoreTypeFamily.TIMESTAMP.equals(lf))
                        return TypeDescriptor.TIMESTAMP;
                    // TIME ± duration is computed in the seconds-of-day domain, so a convertible
                    // encoding
                    // (seconds-from-midnight, HHMMSS integer, 'HH:MM:SS' text) yields a
                    // seconds-from-midnight
                    // result (floor-mod-decoded to a LocalTime). A native TIME column keeps its own
                    // type
                    ...
                    if (!minus && CoreTypeFamily.DATE.equals(lf) && CoreTypeFamily.TIME.equals(rf))
                        return TypeDescriptor.TIMESTAMP;
                    if (minus && isTemporal(lf) && lf.equals(rf)) {
                        // TIMESTAMP − TIMESTAMP is an exact elapsed span → fixed seconds (rendered
                        // HH:MM:SS,
                        // never calendar days, so >24h shows 50:00:00). DATE − DATE is a whole
                        // number of
                        // days. TIME − TIME stays a plain INTERVAL (its natural clock amount).
                        if (CoreTypeFamily.TIMESTAMP.equals(lf)) {
                            return INTERVAL_SECONDS;
                        }
                        return CoreTypeFamily.DATE.equals(lf)
                                ? INTERVAL_DAYS
                                : TypeDescriptor.INTERVAL;
                    }
                    if (CoreTypeFamily.INTERVAL.equals(lf) && rightInterval)
                        return TypeDescriptor.INTERVAL;
                }
    ...
    private static boolean isTemporal(TypeFamily f) {
        return CoreTypeFamily.DATE.equals(f)
                || CoreTypeFamily.TIME.equals(f)
                || CoreTypeFamily.TIMESTAMP.equals(f);
    }
    ...
    private static boolean durationHasClock(Expression operand) {
        Duration d = literalDuration(operand);
        if (d == null) {
            return true; // a non-literal interval (column/expr) — assume a clock part (TIMESTAMP)
        }
        return d.getComponents().stream()
                .anyMatch(
                        c ->
                                c.unit() == Duration.Unit.HOUR
                                        || c.unit() == Duration.Unit.MINUTE
                                        || c.unit() == Duration.Unit.SECOND
                                        || c.unit() == Duration.Unit.MILLISECOND);
    }

    /**
     * The duration an expression denotes, seeing through a scaling factor.
     *
     * <p>{@code 2 * 1d} is the same span as {@code 2d}, but only the second is a duration literal
     * in the tree — the scaling is folded when the expression is rendered, not before. Without
     * this, {@code date + 2d} typed as DATE while {@code date + 2 * 1d} typed as TIMESTAMP, and
     * because the declared family decides whether a read is truncated to a calendar day, the same
     * span gave {@code 2024-04-14} on one dialect and {@code 2024-04-14 00:00:00} on the others.
     *
     * <p>The factor need not be constant: scaling changes how much of a unit is added, never
     * <em>which</em> units appear, and only that governs the question here.
     */
    private static Duration literalDuration(Expression operand) {
        if (operand.getDuration() != null) {
            return operand.getDuration();
        }
        Function fn = operand.getFunction();
        if (fn == null || !MathOp.multiply.name().equals(fn.getFunc())) {
            return null;
        }
        Duration found = null;
        for (Expression arg : fn.getArguments()) {
            if (arg.getDuration() != null) {
                if (found != null) {
                    return null; // duration × duration is not a scaling
                }
                found = arg.getDuration();
            }
        }
        return found;
    }

    public static final ReturnTypeInference TEXT_FROM_BOOLEAN =
            b ->
                    new TypeDescriptor(
                            TypeNames.TYPE_TEXT,
                            CoreTypeEncoding.TEXT_FROM_BOOLEAN,
                            CoreTypeFamily.TEXT);
}
