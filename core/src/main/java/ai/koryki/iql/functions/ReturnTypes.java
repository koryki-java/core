...
public class ReturnTypes {
    ...
    private static final TypeDescriptor INTERVAL_DAYS =
            new TypeDescriptor(TypeNames.TYPE_INTERVAL, new IntervalTypeEncoding(ChronoUnit.DAYS), CoreTypeFamily.INTERVAL);
    ...
    public static final ReturnTypeInference ADD_SUB = binding -> {
        if (binding.getOperandCount() >= 2) {
            ...
            if (minus && isTemporal(lf) && lf.equals(rf)) {
                ...
                // never calendar days, so >24h shows 50:00:00). DATE − DATE is a whole number of
                // days. TIME − TIME stays a plain INTERVAL (its natural clock amount).
                if (CoreTypeFamily.TIMESTAMP.equals(lf)) {
                    return INTERVAL_SECONDS;
                }
                return CoreTypeFamily.DATE.equals(lf) ? INTERVAL_DAYS : TypeDescriptor.INTERVAL;
            }
     ...
     *
     * <p>{@code 2 * 1d} is the same span as {@code 2d}, but only the second is a duration literal in
     * the tree — the scaling is folded when the expression is rendered, not before. Without this,
     * {@code date + 2d} typed as DATE while {@code date + 2 * 1d} typed as TIMESTAMP, and because
     * the declared family decides whether a read is truncated to a calendar day, the same span gave
     * {@code 2024-04-14} on one dialect and {@code 2024-04-14 00:00:00} on the others.
     *
