...
public final class DateTimeFunctions {
    ...
    public static void register(FunctionRegistry r) {
        r.register(
                def("now", ReturnTypes.TIMESTAMP)
                        .args()
                        .template("CURRENT_TIMESTAMP")
                        .doc(
                                "Statement timestamp — fixed for the whole statement "
                                        + "(PostgreSQL `statement_timestamp()`, otherwise `CURRENT_TIMESTAMP`)."));
        r.register(
                def("clock_now", ReturnTypes.TIMESTAMP)
                        .args()
                        .template("CURRENT_TIMESTAMP")
                        .doc(
                                "Wall-clock timestamp read at the moment of the call. Unlike `now`, it may advance "
        ...
        r.register(
                def("quarter", ReturnTypes.INTEGER)
                        .args(arg("value", TEMPORAL, "the date or timestamp to extract from"))
                        .doc("Quarter of the year, 1-4."));
        r.register(
                def("week", ReturnTypes.INTEGER)
                        .args(arg("value", TEMPORAL, "the date or timestamp to extract from"))
                        .doc(
                                "ISO-8601 week of the year, 1-53. Week 1 is the one containing the first "
                                        + "Thursday, so early January can fall in week 52 or 53 of the previous year."));
        r.register(
                def("dayofweek", ReturnTypes.INTEGER)
                        .args(arg("value", TEMPORAL, "the date or timestamp to extract from"))
                        .doc(
                                "Day of the week the ISO way: **Monday is 1** and Sunday is 7. Engines disagree "
        ...
        r.register(
                def("dayofyear", ReturnTypes.INTEGER)
                        .args(arg("value", TEMPORAL, "the date or timestamp to extract from"))
        ...
        r.register(
                def("year_month", ReturnTypes.INTEGER)
                        .args(
                                arg(
                                        "value",
                                        TEMPORAL,
                                        "the date or timestamp to take the year and month of"))
                        .template("(year({0}) * 100 + month({0}))")
                        .doc(
                                "Year and month packed into one number, `YYYYMM`: "
                                        + "year_month('2024-01-15') = 202401.")
                        .paragraph(
                                "A compact bucket key for monthly grouping: it sorts chronologically on "
        ...
        String maskNote =
                "The *format* mask is written in KQL's own vocabulary and translated per "
                        + "dialect, like `to_char`'s — see the *Formatting Functions* page for the tokens.";
        r.register(
                def("parse_date", ReturnTypes.DATE)
                        .args(
                                arg("value", TEXT, "the text to parse"),
                                arg("format", TEXT, "the format mask"))
                        .doc("Parses *value* into a date using the *format* mask.")
                        .paragraph(maskNote));
        r.register(
                def("parse_time", ReturnTypes.TIME)
                        .args(
                                arg("value", TEXT, "the text to parse"),
                                arg("format", TEXT, "the format mask"))
                        ...
                        .paragraph(maskNote));
        r.register(
                def("parse_timestamp", ReturnTypes.TIMESTAMP)
                        .args(
                                arg("value", TEXT, "the text to parse"),
                                arg("format", TEXT, "the format mask"))
                        ...
                        .paragraph(maskNote));
        r.register(
                def("date_trunc", ReturnTypes.ARG1)
                        .args(
                                arg("part", TEXT, "the precision to truncate to, e.g. 'month'"),
        ...
        r.register(
                def("make_timestamp", ReturnTypes.TIMESTAMP)
                        .args(
                                arg("year", INT, "the calendar year"),
                                arg("month", INT, "the month of year, 1-12"),
                                arg("day", INT, "the day of month, 1-31"),
        ...
        r.register(
                def("days_between", ReturnTypes.INTEGER)
                        .args(
                                arg("start", TEMPORAL, "the start of the span"),
                                arg("end", TEMPORAL, "the end of the span"))
                        .template("CAST(trunc(date_diff('second', {0}, {1}) / 86400.0) AS INTEGER)")
                        .doc(
                                "Signed number of **complete days** from *start* to *end* — whole 24-hour "
                                        + "periods that fit inside the interval, not midnights crossed.")
                        .paragraph(
                                "For two dates the two readings coincide, which is why the difference "
        ...
        r.register(
                def("months_between", ReturnTypes.INTEGER)
                        .args(
                                arg("start", TEMPORAL, "the start of the span"),
                                arg("end", TEMPORAL, "the end of the span"))
                        .template(
                                "(CAST(EXTRACT(YEAR FROM age({1}, {0})) * 12 + EXTRACT(MONTH FROM age({1}, {0})) AS INTEGER))")
                        .doc(
                                "Signed number of whole months completed from *start* to *end*; "
                                        + "months_between('2023-01-31', '2023-03-30') = 1 — the second month is not complete."));
        r.register(
                def("years_between", ReturnTypes.INTEGER)
                        .args(
                                arg("start", TEMPORAL, "the start of the span"),
                                arg("end", TEMPORAL, "the end of the span"))
        ...
        // calendar_distance: the *variable* (civil) decomposition into months/days + clock —
        // complements
        // the fixed elapsed-seconds difference (ts − ts). SQL only emits the two instants as
        // epoch-seconds
        // ("start;end"); the calendar decomposition (Period.between, reference java.time semantics)
        // happens
        ...
        r.register(
                new FunctionDefinition("calendar_distance", ReturnTypes.CALENDAR_DISTANCE) {
                    @Override
                    protected String renderBody(
                        ...
                        String s =
                                d.epochSeconds(
                                        renderer.toSql(start, indent), renderer.resolveType(start));
                        String e =
                                d.epochSeconds(
                                        renderer.toSql(end, indent), renderer.resolveType(end));
                        ...
                        .doc(
                                "Calendar (civil) distance from *start* to *end* as a mixed interval "
        ...
        r.register(
                def("minute_begin", ReturnTypes.ARG0)
                        .args(
                                arg(
                                        "value",
                                        TEMPORAL,
                                        "the date or timestamp to floor to the minute"))
                        .template("date_trunc('minute', {0})")
                        .doc("Start of the minute (seconds become 00)."));
        r.register(
                def("hour_begin", ReturnTypes.ARG0)
                        .args(arg("value", TEMPORAL, "the date or timestamp to floor to the hour"))
                        .template("date_trunc('hour', {0})")
                        .doc("Start of the hour (minutes and seconds become 00).")
                        .paragraph(
                                "The bucket for sub-daily data, and the one the family was missing: with "
        ...
        r.register(
                def("day_begin", ReturnTypes.ARG0)
                        .args(arg("value", TEMPORAL, "the date or timestamp to floor to the day"))
                        .template("date_trunc('day', {0})")
                        .doc("Start of the day (time becomes 00:00:00)."));
        r.register(
                def("week_begin", ReturnTypes.ARG0)
                        .args(arg("value", TEMPORAL, "the date or timestamp to floor to the week"))
                        .template("date_trunc('week', {0})")
                        .doc(
                                "Monday of the week (pairs with week_end). Grouping by this is the reliable way "
        ...
        r.register(
                def("month_begin", ReturnTypes.ARG0)
                        .args(arg("value", TEMPORAL, "the date or timestamp to floor to the month"))
                        .template("date_trunc('month', {0})")
                        .doc("First day of the month (pairs with month_end)."));
        r.register(
                def("quarter_begin", ReturnTypes.ARG0)
                        .args(
                                arg(
                                        "value",
                                        TEMPORAL,
                                        "the date or timestamp to floor to the quarter"))
                        .template("date_trunc('quarter', {0})")
                        .doc("First day of the quarter."));
        r.register(
                def("year_begin", ReturnTypes.ARG0)
                        .args(arg("value", TEMPORAL, "the date or timestamp to floor to the year"))
        ...
        r.register(
                def("week_end", ReturnTypes.DATE)
                        .args(
                                arg(
                                        "value",
                                        TEMPORAL,
                                        "the date or timestamp to take the week end of"))
                        .template("CAST(date_trunc('week', {0}) + INTERVAL 6 DAY AS DATE)")
                        .doc("Sunday of the week."));
        r.register(
                def("month_end", ReturnTypes.DATE)
                        .args(
                                arg(
                                        "value",
                                        TEMPORAL,
                                        "the date or timestamp to take the month end of"))
                        .template("last_day({0})")
                        .doc(
                                "Last day of the month (Excel EOMONTH): month_end('2024-02-10') = 2024-02-29."));
        r.register(
                def("quarter_end", ReturnTypes.DATE)
                        .args(
                                arg(
                                        "value",
                                        TEMPORAL,
                                        "the date or timestamp to take the quarter end of"))
                        .template("last_day(date_trunc('quarter', {0}) + INTERVAL 2 MONTH)")
                        .doc("Last day of the quarter."));
        r.register(
                def("year_end", ReturnTypes.DATE)
                        .args(
                                arg(
                                        "value",
                                        TEMPORAL,
                                        "the date or timestamp to take the year end of"))
        ...
        r.register(
                new ZoneShiftFunctionDefinition("at_zone", true)
                        .category(FunctionCategory.DATETIME)
                        .doc(
                                "Reads *value* as a model-zone wall-clock value and returns its wall-clock in the named "
                                        + "zone (e.g. for day-bucketing: date(at_zone(o.ts, 'Europe/Berlin')))."));
        r.register(
                new ZoneShiftFunctionDefinition("to_utc", false)
                        .category(FunctionCategory.DATETIME)
                        .doc(
                                "Inverse of at_zone: reads *value* as a wall-clock in the named zone and returns the "
                                        + "model-zone wall-clock value."));
    }

    /**
     * ANSI {@code EXTRACT(part FROM x)} rendering for the date-part functions; opted into by
     * dialects.
     *
    ...
    public static void registerExtractParts(FunctionRegistry r) {
        for (String[] entry :
            ...
            r.register(
                    def(entry[0], ReturnTypes.INTEGER)
                            .args(arg("value", TEMPORAL, "the date or timestamp to extract from"))
     ...
     * that accept the canonical KQL mask as-is (PostgreSQL, Oracle, Snowflake — see {@link
     * ai.koryki.iql.functions.FormatMask}).
     */
     ...
     */
    public static FunctionDefinition parseWithMask(
            String name,
            ReturnTypeInference type,
            UnaryOperator<String> mask,
        ...
        return new FunctionDefinition(name, type) {
            ...
            protected String renderBody(SqlSelectRenderer renderer, Function function, int indent) {
                ...
                return sql.apply(
                        renderer.toSql(args.get(0), indent),
