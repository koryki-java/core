...
/**
 * The conditional core: three primitives that render to standard SQL on every dialect — {@code
 * coalesce} (first non-null), {@code nullif} (its inverse) and the searched {@code case}.
 *
 ...
 *
 * <p>{@code nullif} is the one that does not follow strictly from that rule: {@code case(a = b,
 * null, a)} expresses it, so by the principle above it need not be here. It is kept because it is
 * standard SQL, carries no dialect ambiguity, and states the intent — turning a placeholder into a
 * real blank — far more plainly than the {@code case} that replaces it.
 */
public final class ConditionalFunctions {
    ...
    public static void register(FunctionRegistry r) {
        r.register(
                new ConditionalFunctionDefinition("coalesce")
                        .category(FunctionCategory.CONDITIONAL)
                        .variadic(
                                arg(
                                        "value",
                                        Families.ANY,
                                        "the values checked in order for the first non-null"))
                        .doc(
                                "The first argument that is not blank; blank only if every argument is.")
                        .paragraph(
                                "The arguments are read left to right and **evaluation stops at the first "
        ...
        r.register(
                new ConditionalFunctionDefinition("nullif")
                        .category(FunctionCategory.CONDITIONAL)
                        .args(
                                arg("value", Families.ANY, "the value to return unless it matches"),
                                arg(
                                        "when",
                                        Families.ANY,
                                        "the value that turns the result into null"))
                        .doc(
                                "*value*, or blank when it equals *when* — the inverse of `coalesce`, and the way "
                                        + "to turn a placeholder such as `0` or an empty string into a real blank.")
                        .paragraph(
                                "The two are **compared**, so they must be able to meet in one type, and the "
                                        ...
                                        + "number, is an error."));
        r.register(
                new CaseFunctionDefinition()
                        .category(FunctionCategory.CONDITIONAL)
                        .variadic(
                                arg(
                                        "condition",
                                        CoreTypeFamily.BOOLEAN,
                                        "a WHEN test evaluated in order until one is true"),
                                arg(
                                        "result",
                                        Families.ANY,
                                        "the value returned when its paired condition is true"))
                        .doc(
                                "Tests each condition in order and returns the result paired with the first one "
                                        + "that holds. A trailing argument without a condition is the fallback; without "
                                        + "it, a row that matches nothing comes out blank.")
                        .paragraph(
                                "All the results must be able to meet in **one type** — that is what the "
