...
public final class ConditionalReconciler {
    ...
    public record Result(TypeDescriptor target, List<Conversion> perBranch) {
        /**
         * Wrap branch {@code i}'s rendered SQL in its reconciliation conversion (identity →
         * unchanged), using only dialect-neutral SQL.
         *
         * <p>Prefer {@link #convert(SqlSelectRenderer, Result, int, String)} wherever a renderer is
         * at hand: some conversions have no portable form.
         */
    ...
    /**
     * Branch {@code i}'s conversion, rendered by the dialect where the lattice's neutral SQL will
     * not do.
     *
     * <p>Unpacking an encoded TIME to seconds-of-day is the case in point: {@code EXTRACT(EPOCH …)}
     * is absent on MariaDB and SQLite, {@code MOD} on SQL Server, and Oracle has no {@code TIME}
     * type at all. {@link EncodingLattice} still decides <em>whether</em> two encodings meet and
     * where — only the rendering is the dialect's.
     */
    public static String convert(SqlSelectRenderer renderer, Result result, int i, String sql) {
        Conversion c = result.perBranch().get(i);
        if (c.isIdentity()) {
            return sql;
        }
    ...
    public static final class ReconcileException extends RuntimeException {
        ...
        public ReconcileException(Kind kind, String message) {
            super(message);
            this.kind = kind;
        }
