...
public final class StringFunctions {
    ...
    public static void register(FunctionRegistry r) {
        ...
        // different unit.
        r.register(
                def("char_length", ReturnTypes.INTEGER)
                        .args(arg("string", TEXT, "the text to measure"))
                        .doc("Number of characters in the string."));
        r.register(
                def("octet_length", ReturnTypes.INTEGER)
                        .args(arg("string", TEXT, "the text to measure"))
                        .doc("Number of bytes in the string."));
        r.register(
                def("bit_length", ReturnTypes.INTEGER)
                        .args(arg("string", TEXT, "the text to measure"))
        ...
        String asciiOnly =
                "On SQLite this folds ASCII only, so accented letters are returned "
                        + "unchanged; every other supported engine is Unicode-aware.";
        r.register(
                def("upper", ReturnTypes.TEXT)
                        .args(arg("string", TEXT, "the text to convert"))
                        .doc("Converts the string to upper case.")
                        .paragraph(asciiOnly));
        r.register(
                def("lower", ReturnTypes.TEXT)
                        .args(arg("string", TEXT, "the text to convert"))
                        .doc("Converts the string to lower case.")
                        .paragraph(asciiOnly));
        r.register(
                def("initcap", ReturnTypes.TEXT)
                        .args(arg("string", TEXT, "the text to capitalize"))
                        .doc(
        ...
        r.register(
                def("trim", ReturnTypes.TEXT)
                        .args(
                                arg("string", TEXT, "the text to trim"),
                                optionalArg(
                                        "characters",
                                        TEXT,
                                        "the characters to strip; spaces if omitted"))
                        .doc("Removes *characters* (default spaces) from both ends.")
                        .paragraph(
                                "*characters* is a **set of characters**, not a substring: `trim('abhelloba', "
        ...
        {
            r.register(
                    def("substring", ReturnTypes.TEXT)
                            .args(
                                    arg("string", TEXT, "the source text"),
                                    arg(
                                            "start",
                                            INT,
                                            "1-based index of the first character to keep"),
                                    optionalArg(
                                            "length",
                                            INT,
                                            "number of characters to take; to the end of the string if omitted"))
                            .doc(
                                    "Extracts the substring starting at *start* (1-based), optionally limited to "
                                            + "*length* characters. Without *length* it runs to the end of the string.")
                            .paragraph(
                                    "*start* counts from **1**. A value below that is outside the definition and "
        ...
        r.register(
                def("left", ReturnTypes.TEXT)
                        .args(
                                arg("string", TEXT, "the source text"),
                                arg("n", INT, "number of leading characters to keep"))
                        .doc("First *n* characters."));
        r.register(
                def("right", ReturnTypes.TEXT)
                        .args(
                                arg("string", TEXT, "the source text"),
                                arg("n", INT, "number of trailing characters to keep"))
                        .doc(
                                "Last *n* characters — the whole string if *n* reaches or exceeds its length, "
                                        + "and nothing at all if *n* is zero.")
                        .paragraph(
                                "Those two edges are worth stating because three dialects express `right` "
        ...
        r.register(
                def("reverse", ReturnTypes.TEXT)
                        .args(arg("string", TEXT, "the text to reverse"))
                        .doc("Reverses the string."));
        r.register(
                def("repeat", ReturnTypes.TEXT)
                        .args(
                                arg("string", TEXT, "the text to repeat"),
                                arg("number", INT, "how many times to repeat the text"))
                        .doc("Repeats the string *number* times."));

        r.register(
                def("lpad", ReturnTypes.TEXT)
                        .args(
                                arg("string", TEXT, "the text to pad"),
        ...
        r.register(
                def("concat", ReturnTypes.TEXT)
                        .variadic(arg("value", Families.ANY, "the values to concatenate"))
                        .doc(
                                "Concatenates the text form of all arguments; null arguments are ignored."));
        r.register(
                def("concat_ws", ReturnTypes.TEXT)
                        .variadic(
                                arg(
                                        "separator",
                                        TEXT,
                                        "the separator placed between the joined values"))
                        .doc(
                                "Concatenates all arguments after the first, separated by *separator*."));

        r.register(
                def("replace", ReturnTypes.TEXT)
                        .args(
                                arg("string", TEXT, "the source text"),
        ...
        r.register(
                def("translate", ReturnTypes.TEXT)
                        .args(
                                arg("string", TEXT, "the source text"),
                ...
                def("overlay", ReturnTypes.TEXT)
                        .args(
                                ...
                                optionalArg(
                                        "length",
                                        INT,
                                        "number of characters to overwrite; the replacement's length if omitted"))
                        .doc(
                                "Writes *replacement* over *string*, beginning at *start* and covering *length* "
        ...
        r.register(
                def("starts_with", ReturnTypes.BOOLEAN)
                        .args(
                                arg("string", TEXT, "the text to test"),
                                arg("prefix", TEXT, "the prefix to look for"))
                        .doc("True if the string begins with *prefix*."));
        r.register(
                def("split_part", ReturnTypes.TEXT)
                        .args(
                                arg("string", TEXT, "the text to split"),
        ...
        r.register(
                def("ascii", ReturnTypes.INTEGER)
                        .args(arg("character", TEXT, "the text whose first character is coded"))
                        .doc("Numeric code of the first character.")
                        .paragraph(
                                "**Defined for ASCII.** Beyond it the databases part company, because they do not agree what "
        ...
        r.register(
                def("chr", ReturnTypes.TEXT)
                        .args(arg("code", INT, "the character code to convert"))
                        ...
                        .paragraph(
                                "**Defined for ASCII.** Beyond it the databases part company, because they do not agree what "
                                        ...
                                        + "the characters in the application."));
        r.register(
                def("to_hex", ReturnTypes.TEXT)
                        .args(arg("number", INT, "the number to convert to hexadecimal"))
                        .doc(
                                "Hexadecimal representation of the number, in **upper case** and without a "
                                        + "leading `0x`.")
                        .paragraph(
                                "A negative number comes out as its two's-complement form, and how wide that "
                                        + "is depends on the database's integer width — so a negative input is not "
                                        + "portable. Positive values agree everywhere."));
        r.register(
                def("md5", ReturnTypes.TEXT)
                        .args(arg("string", TEXT, "the text to hash"))
    ...
    public static FunctionDefinition positionFunction() {
        return def("position", ReturnTypes.INTEGER)
                .args(
                        arg("substr", TEXT, "the substring to locate"),
