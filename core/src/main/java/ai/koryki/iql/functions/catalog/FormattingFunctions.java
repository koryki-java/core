 ...
 * — {@code YYYY YY MM DD HH24 HH12 HH MI SS AM PM}, the PostgreSQL template patterns, which
 * PostgreSQL inherits from Oracle — and {@link ai.koryki.iql.functions.FormatMask} rewrites it into
 * each dialect's own token language: strftime {@code %}-codes for DuckDB and SQLite, DATE_FORMAT
...
public final class FormattingFunctions {
    ...
    public static void register(FunctionRegistry r) {
        r.register(
                def("to_char", ReturnTypes.TEXT)
                        .args(
                                arg("value", Families.ANY, "the date or timestamp to format"),
                                arg(
                                        "format",
                                        CoreTypeFamily.TEXT,
                                        "the format mask, written once in KQL's own vocabulary"))
                        .doc("Formats a date or timestamp as text using the *format* mask.")
                        .paragraph(
                                "The mask is written **once**, in KQL's vocabulary, and translated for each "
                                        ...
                                        + "of raising an error."));
        r.register(
                def("to_number", ReturnTypes.DECIMAL)
                        .args(
                                arg(
                                        "value",
                                        CoreTypeFamily.TEXT,
                                        "the text to parse into a number"),
                                arg(
                                        "format",
                                        CoreTypeFamily.TEXT,
                                        "a numeric template such as `999D99`"))
                        .doc("Parses a string into a number using a numeric *format* mask.")
                        .paragraph(
                                "This is **not** the datetime vocabulary above and is not translated: the "
