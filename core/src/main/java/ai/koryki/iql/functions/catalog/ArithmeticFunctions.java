...
/**
 * Internal arithmetic operator functions (add, minus, multiply, divide, negate). Rendering stays in
 * Java: operands chain pairwise through the dialect's {@code renderEncodedArithmetic} hook so
 * encoded types (e.g. TIME stored as seconds-from-midnight) can intercept each step.
 */
public final class ArithmeticFunctions {
    ...
    public static void register(FunctionRegistry r) {
        r.register(
                new FunctionDefinition(MathOp.negate.name(), ReturnTypes.ARG0) {
                    @Override
                    protected String renderBody(
                        ...
                        //
                        // A multi-part DURATION needs them too: it renders as "a + b", and a
                        // leading minus
                        // bound to the first part only -- -2d4h became -2d + 4h, eight hours off
                        // without an
                        // error. Measured on oracle, mariadb and trino.
                        boolean needsParens =
                                !operand.isParenthesized()
                                        && (operand.getFunction() != null
                        ...
                        .doc(
                                "Arithmetic negation (`-x`): negates a number, or flips the sign of every component "
                                        + "of a DURATION."));
        ...
        for (MathOp op : new MathOp[] {MathOp.add, MathOp.minus, MathOp.multiply, MathOp.divide}) {
            ...
            FunctionDefinition fn =
                    new FunctionDefinition(op.name(), returnType) {
                        @Override
                        protected String renderBody(
                                SqlSelectRenderer renderer, Function function, int indent) {
                            ...
                            if (current == MathOp.multiply && args.size() == 2) {
                                // DURATION × integer (either order) → the component-wise scaled
                                // duration literal,
                                // rendered via the dialect's own duration rendering (portable;
                                // avoids `n * INTERVAL`).
                                String scaled =
                                        scaleDurationLiteral(
                                                renderer, args.get(0), args.get(1), indent);
                                if (scaled != null) {
                                    return scaled;
                                }
                            }
                            ...
                            String result =
                                    step(
                                            ...
                                            current,
                                            symbol,
                                            renderer.toSql(leftExpr, indent),
                                            leftType,
                                            rightExpr,
                                            rightType,
                                            indent);
     ...
     * undefined), and it needs the integer-division and divide-by-zero normalisations that {@link
     * ai.koryki.iql.SqlDialect#renderDivision} owns.
     */
        ...
        if (op == MathOp.divide) {
            return renderer.getDialect()
                    .renderDivision(
                            leftSql,
                            leftType,
                            renderer.toSql(right, indent),
                            rightType,
                            needsZeroGuard(right));
        }
        return renderer.getDialect()
                .renderEncodedArithmetic(
                        renderer, symbol, leftSql, leftType, right, rightType, indent);
    }
    ...
    /**
     * {@code integer × DURATION-literal} (either order) folded into the component-wise scaled
     * duration (per docs/TEMPORAL.md), rendered through the dialect's normal duration rendering —
     * so it is portable and never emits {@code n * INTERVAL}. Returns {@code null} when the pair is
     * not an integer-scaled duration literal (e.g. a column factor, a non-integer factor, or
     * duration × duration), leaving the caller to fall back to the generic numeric multiply.
     */
    private static String scaleDurationLiteral(
            SqlSelectRenderer renderer, Expression a, Expression b, int indent) {
        Expression durExpr;
        Expression numExpr;
    ...
    /**
     * Structural precedence guard for hand-written IQL: a nested arithmetic call whose SQL grouping
     * would be re-associated by operator precedence gets the parenthesized flag (which the renderer
     * turns into parens). Explicit user parens already carry the flag — this only adds the parens
     * infix rendering cannot do without.
     */
