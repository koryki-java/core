...
/**
 * Comparison operators as first-class catalog entries — the data half of unifying functions and
 * operators. The logical connectives live in {@link LogicalFunctions}.
 *
 * <p><b>Phase 2 (this class):</b> operators are registered for type metadata, validation and
 * documentation only. The renderer still handles comparisons directly via {@code
 * SqlSelectRenderer}, so registering these definitions is <em>behaviour-neutral</em> for SQL
 * generation. A later phase flips the renderer to dispatch through the catalog by the operator's
 * surface text.
 *
 * <p>Keys are exactly the text the KQL mapper stores in {@code UnaryLogicalExpression.op} (the
 * matched token text: {@code "="}, {@code ">="}, {@code "BETWEEN"}, {@code "ISNULL"}, …), so a
 * future renderer can resolve an operator by that text. The grammar's {@code operator: … |
 * custom=ID} rule makes the operator set open-ended — a custom operator is just another entry here,
...
public final class ComparisonFunctions {
    ...
    public static void register(FunctionRegistry r) {
        // Comparison operators — the grammar's `operator` rule, plus BETWEEN, which has a
        // production
        // of its own so that its AND cannot be read as the logical one. Operands are generic (any
        // value compares to
        ...
        r.register(
                comparison("<>", Fixity.INFIX, "{0} <> {1}")
                        .args(
                                ...
                                arg("right", Families.ANY, "the value to compare against"))
                        .doc(
                                "True if *left* does not equal *right*. NULL on either side yields NULL, "
                ...
                comparison(">=", Fixity.INFIX, "{0} >= {1}")
                        .args(
                                ...
                                arg(
                                        "right",
                                        Families.ORDERED,
                                        "the threshold it must reach or exceed"))
                        .doc("True if *left* is greater than or equal to *right*."));
        r.register(
                comparison("LIKE", Fixity.INFIX, "{0} LIKE {1}")
                        .args(
                                arg("string", CoreTypeFamily.TEXT, "the text being matched"),
                                arg(
                                        "pattern",
                                        CoreTypeFamily.TEXT,
                                        "the SQL LIKE pattern with `%` and `_` wildcards"))
                        .doc(
                                "True if *string* matches the SQL LIKE *pattern* (`%` and `_` wildcards). "
                                        + "Both operands must be TEXT."));
        r.register(
                comparison("BETWEEN", Fixity.RANGE, "{0} BETWEEN {1} AND {2}")
                        .args(
        ...
        r.register(
                comparison("IN", Fixity.SET, "{0} IN ({1*})")
                        .variadic(
        ...
        // false even when an operand is NULL, where the others answer unknown and the row is
        // dropped.
        r.register(
                comparison("DISTINCT", Fixity.INFIX, "{0} IS DISTINCT FROM {1}")
                        .args(
                                arg("left", Families.ANY, "the value to compare"),
                                arg("right", Families.ANY, "the value to compare it against"))
                        .doc(
                                "True if *left* and *right* hold different values, **counting a missing value as "
                        ...
                        .paragraph(
                                "This is the same distinctness `FETCH DISTINCT` applies when it collapses "
        ...
        r.register(
                comparison("ISNULL", Fixity.POSTFIX, "{0} IS NULL")
