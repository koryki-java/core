...
/** Replace Identity-Indicator with first PK-Field. This is used for count(ID). */
public class IdentityRule {
    ...
    public void apply(Query query) {
        ...
        // Only now: the walk iterates a function's argument list, and count_distinct grows it.
        // Doing
    ...
    private static class IdentityVisitor implements Visitor {
        ...
        public boolean visit(Deque<Object> deque, Expression expression) {
            ...
            if (expression.getIdentity() != null) {
                Visitor.getNthElement(deque, 1)
                        .map(e -> e instanceof Function ? (Function) e : null)
                        .ifPresent(
                                f -> {
                                    Source table =
                                            Visitor.findSourceInParentSelect(
                                                    deque, expression.getIdentity());
                                    String alias = table.getAlias();

                                    enrichPkColumn(table, expression, alias, 1);

                                    // count_distinct asks how many *different* rows, and rows
                                    // differ by their whole
                                    // key. With one column it answered a different question: over
                                    // order_details,
                                    // keyed by (order_id, product_id), COUNT(DISTINCT order_id)
                                    // counted orders --
                                    // 830 where the truth is 2155, and nothing said so.
                                    //
                                    // count deliberately keeps the single column above: a key is
                                    // never null, so
                                    // counting one counts rows, and under an outer join it counts 0
                                    // for an unmatched
                                    // row where COUNT(*) would count 1.
                                    if ("count_distinct".equals(f.getFunc())) {
                                        for (int pos = 2; pos <= keyCount(table); pos++) {
                                            // Through the same method as the first column,
                                            // deliberately: it is the
                                            // one that follows a block to its leading source and
                                            // projects what the
                                            // outer query needs. A second implementation would have
                                            // to relearn that.
        ...
        private String enrichPkColumn(
                Source source, Expression expression, String tableAlias, int pkPos) {
            Source blockSource = blockIdToLeadingSourceMap.get(source.getName());
            if (blockSource != null) {
                String column = enrichPkColumn(blockSource, expression, tableAlias, pkPos);

                String h =
                        blockSource.getOut().stream()
                                .filter(
                                        o ->
                                                o.getHeader() != null
                ...
                Optional<ai.koryki.catalog.schema.Table> ot =
                        resolver.getDialectTable(source.getName())
                                .flatMap(table -> resolver.getSchema().getTable(table));

                // The pkPos-th key column. count asks for the first only -- a key is never null, so
                // counting
                // one column counts rows. count_distinct asks for every one of them, because two
                // rows differ
                // by their whole key: over order_details it is (order_id, product_id), and the
                // first column
                ...
                String col =
                        pkColumn == null
                                ? null
                                : resolver.getModelAttribute(source.getName(), pkColumn)
                                        .orElse(pkColumn);

                String h =
                        source.getOut().stream()
                                .filter(
                                        o ->
                                                o.getHeader() != null
                                                        && o.getExpression().getField() != null
        ...
        private void enrichBlocksource(Source blockSource, String fieldName) {
            ...
            if (blockSource.getOut().stream()
                    .noneMatch(
                            out ->
                                    out.getExpression().getField() != null
                                            && out.getExpression()
                                                    .getField()
                                                    .getName()
                                                    .equals(fieldName))) {
