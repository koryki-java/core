...
public class DuckdbBaseDialect implements SqlDialect {
    ...
    /**
     * DuckDB list literals accept any scalar element except binary (BLOB) and logical (BOOLEAN)
     * values.
     */
    private static final TypeFamily LIST_ELEMENT =
            new FamilyGroup(
                    "LIST_ELEMENT",
                    Set.of(
                            CoreTypeFamily.DATE,
                            CoreTypeFamily.DECIMAL,
                            CoreTypeFamily.FLOAT,
                            CoreTypeFamily.INTEGER,
                            CoreTypeFamily.TIME,
                            CoreTypeFamily.INTERVAL,
                            CoreTypeFamily.TIMESTAMP,
                            CoreTypeFamily.TEXT,
                            CoreTypeFamily.JSON,
                            CoreTypeFamily.UUID));
     ...
     * timestamp and an instant: applying the declared zone reads the naive stored value as an
     * instant, then applying the model zone renders that instant back as a naive model-zone
     * wall-clock value. A {@code DATE_WALLCLOCK} is converted at start-of-day in the declared zone
     * and taken back to a date.
     */
    @Override
    ...
    /**
     * Translates a KQL Oracle-style format string literal (e.g. 'YYYY-MM-DD HH24:MI:SS') to a
     * DuckDB strptime format literal (e.g. '%Y-%m-%d %H:%M:%S'). Non-literal expressions (runtime
    ...
    private static FunctionRenderer buildFunctionRenderer() {
        ...
        registry.register(
                new FunctionDefinition("list", ReturnTypes.TEXT)
                        .args(FunctionArg.arg("value", LIST_ELEMENT))
                        .template("[{0}]"));

        // truncate → trunc
        registry.register(
                new FunctionDefinition("truncate", ReturnTypes.ARG0)
        ...
        // same-named column alias — `SELECT CURRENT_DATE AS current_date` fails to bind — whereas
        // the
        ...
        registry.register(
                new FunctionDefinition(registry.overloads("regexp_count").get(0)) {
                    @Override
        ...
        // overlay(string, replacement, start [, length]): DuckDB has no overlay(); splice with
        // substr.
        registry.register(
        ...
        for (String name : java.util.List.of("bit_and", "bit_or", "bit_xor")) {
            registry.register(
                    new FunctionDefinition(name, ReturnTypes.INTEGER, FunctionKind.AGGREGATE));
        }
        ...
        // | to_timestamp(year, month, day, hour, minute, second) | to_timestamp(year, month, day,
        ...
        registry.register(
                new FunctionDefinition("to_timestamp", ReturnTypes.TIMESTAMP)
                        .args(
                                ...
                                FunctionArg.arg("tz", CoreTypeFamily.TEXT))
                        .template(
                                "timezone({6}, MAKE_TIMESTAMP({0}, {1}, {2}, {3}, {4}, {5})::TIMESTAMPTZ)"));
        ...
        registry.register(
                DateTimeFunctions.parseWithMask(
                        "parse_date",
                        ReturnTypes.DATE,
                        DuckdbBaseDialect::translateKQLFormat,
                        (v, f) -> "strptime(" + v + ", " + f + ")::DATE"));
        registry.register(
                DateTimeFunctions.parseWithMask(
                        "parse_time",
                        ReturnTypes.TIME,
                        DuckdbBaseDialect::translateKQLFormat,
                        (v, f) -> "strptime(" + v + ", " + f + ")::TIME"));
        registry.register(
                DateTimeFunctions.parseWithMask(
                        "parse_timestamp",
        ...
        // unit is one of: YEAR, MONTH, WEEK, DAY, HOUR, MINUTE, SECOND, MILLISECOND
        // (case-insensitive)
        registry.register(
        ...
        registry.register(
                new FunctionDefinition("to_interval", ReturnTypes.INTERVAL)
                        ...
                        .template(
                                "to_years({0}) + to_months({1}) + to_days({2}) + to_hours({3}) + to_minutes({4}) + to_seconds({5})"));
    ...
    public String durationLiteral(ai.koryki.iql.query.Duration duration) {
        return SqlDialect.combinedInterval(
                duration, " "); // DuckDB parses the space-separated verbose form
    }
    ...
    public String renderComparisonOperand(
            ...
            int indent) {
        java.util.Optional<String> seconds =
                TimeEncodings.secondsFromMidnightLiteral(leftType, expression);
        ...
        // midnight (00:00:00)" — and so do the other seven engines, which leave the promotion to
        // the
