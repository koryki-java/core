...
/**
 * Materialized INTERVAL value — the read-side counterpart of the query-AST {@code
 * ai.koryki.iql.query.Duration} (which is never a value; it renders to dialect SQL). The boundary
 * decoder ({@link CoreDecoder} and the dialect decoders) produces this from a native interval
 * object or an encoded column.
 *
 * <p>Holds the three independent fields every supported dialect stores — <b>months, days,
 * nanoseconds</b> — kept separate (not normalized) because months are variable-length and days vary
 * under DST. This losslessly covers Postgres/DuckDB (months+days+µs), Oracle
 * YEAR&nbsp;TO&nbsp;MONTH (months) and Oracle DAY&nbsp;TO&nbsp;SECOND (days+nanos), and the mixed
 * KQL duration {@code 1y2mo3d4h}. Maps to {@code java.time.Period} when it is months/days only, to
 * {@code Duration} when nanos only, and stays a true mixed amount otherwise. Implements {@link
 * TemporalAmount} so it can be added to a {@code Temporal}; {@code LocaleFormat} renders it for
 * display.
 */
public final class Interval implements TemporalAmount {
    ...
    public boolean isZero() {
        return months == 0 && days == 0 && nanos == 0L;
    }

    public Interval negated() {
        return of(-months, -days, -nanos);
    }
    ...
    public String toString() {
        ...
        if (nanos != 0) {
            b.append('T');
            long n = nanos;
            long hours = n / 3_600_000_000_000L;
            n %= 3_600_000_000_000L;
            long mins = n / 60_000_000_000L;
            n %= 60_000_000_000L;
     ...
     * The presentation form (see {@code LocaleFormat}): round-trips with the KQL duration literal
     * and avoids ISO-8601's {@code M} month/minute ambiguity. The smallest emitted unit is the
     * (possibly fractional) second, so a sub-second amount shows as {@code 0.5s}, not {@code
     * 500ms}. Distinct from {@link #toString()}, which stays ISO-8601 for interchange/debug.
     */
    public String toKql() {
        if (isZero()) {
            return "0s";
