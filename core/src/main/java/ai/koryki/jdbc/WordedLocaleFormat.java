...
public class WordedLocaleFormat extends LocaleFormat {
    ...
    /**
     * Applies the column's {@code Presentation} when it has one, and falls through otherwise.
     *
     * <p>Here rather than in {@link LocaleFormat} on purpose. {@code LocaleFormat} is the canonical
     * renderer, and {@code StableFormat} — which the CSV goldens of eight dialects run through —
     * extends it. Putting a display precision there would move every one of those goldens. This
     * class already declares itself the business-facing presentation, so the rule belongs to it and
     * the separation is structural rather than a flag someone has to remember.
     *
     * <p>A presentation that returns {@code null} has nothing to say about this value — a decimal
     * rule meeting a date — and the base rendering applies unchanged.
     */
    @Override
    public String formatColumn(Object value, ColumnInfo info) {
        if (value != null && info != null && info.getPresentation() != null) {
            String rendered = info.getPresentation().render(value, locale());
            if (rendered != null) {
                return rendered;
            }
        }
        return super.formatColumn(value, info);
    }
