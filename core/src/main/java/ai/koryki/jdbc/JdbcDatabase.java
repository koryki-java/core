...
public class JdbcDatabase<C extends ResultProcessor<?>> implements Database<C> {
    ...
    /**
     * Render {@code zone} as an IANA name / offset for dialects whose SET accepts it (default UTC →
     * "UTC").
     */
    ...
    /**
     * Render {@code zone} as a numeric UTC offset (no tz tables needed); falls back to the name for
     * a region zone.
     */
    ...
    private Object read(ResultSet rs, int i, int jdbcType, ColumnInfo info) throws SQLException {
        ...
        // column type per row, so a NULL in an otherwise-textual column comes back typed NUMERIC
        // and
        ...
        switch (jdbcType) {
            ...
            case Types.TIMESTAMP:
                ...
                v = timestamp != null ? timestamp.toLocalDateTime() : null;
                break;
     ...
     * of the JVM default zone. The default asks the driver for an {@link OffsetDateTime} (JDBC
     * 4.2), which is correct for the zone-aware native types — Postgres {@code timestamptz}, SQL
     * Server {@code datetimeoffset}, Oracle {@code TIMESTAMP WITH TIME ZONE}, DuckDB {@code
     * TIMESTAMPTZ}.
     *
     ...
     *       wall-clock already <em>is</em> the UTC value, and {@code
     *       getObject(OffsetDateTime.class)} would wrongly stamp it with the JVM offset.
     *   <li>SQLite stores it as text and does not support {@code getObject(OffsetDateTime.class)}.
     *   <li>Trino exposes the instant with the type of whatever backend it federates, so it
     *       branches on {@code jdbcType} — hence that parameter is part of the contract.
     * </ul>
     *
     ...
     * wall-clock (the session is pinned to {@link #getModelZone()}). {@code toLocalDateTime()}
     * recovers that wall-clock JVM-zone-independently; reinterpreting it at the model zone yields
     * the instant. Used by MariaDB, and by Trino when it federates a backend whose instant maps to
     * a naive {@code timestamp}.
     */
    ...
    protected void setConnection(Connection conn) {
        this.connection = conn;
        // A swapped-in connection (e.g. a reconnect after an error) is born with the driver's
        // default
        // session zone, so re-pin it to the model zone — otherwise zone-dependent reads (Trino
        // from_unixtime, now(), …) silently drift to the client JVM zone for the rest of the
        // session.
