...
/**
 * Parses the two SQL-standard interval spellings a driver may hand back as plain text: {@code
 * "[-]Y-M"} for YEAR TO MONTH and {@code "[-]D H:M:S[.fff]"} for DAY TO SECOND.
 *
 ...
 * engine produced a koryki {@link Interval}. That was visible only as two fixtures whose {@code
 * ignore=trino} marker said "results differ" — they did not differ in value, only in spelling.
 *
...
public final class IntervalStrings {
    ...
    public static Interval parse(String s) {
        ...
        if (dash > 0
                && body.indexOf(' ') < 0
                && digitsOnly(body.substring(0, dash))
    ...
    public static Interval parseDaySecond(String s) {
        ...
        long nanos =
                Long.parseLong(hms[0].trim()) * 3_600_000_000_000L
