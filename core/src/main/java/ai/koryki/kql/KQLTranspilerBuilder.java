...
public final class KQLTranspilerBuilder {
    ...
    private ai.koryki.iql.SqlDialect dialect;
    ...
    /**
     * Dialect whose validators run alongside the core ones — it rejects constructs it cannot
     * express (see {@link ai.koryki.iql.SqlDialect#validators}). Null (the default) skips them.
     *
     * <p>Pass the same dialect the query will be rendered with; otherwise the analysis approves a
     * query the renderer's target cannot run.
     */
    public KQLTranspilerBuilder dialect(ai.koryki.iql.SqlDialect dialect) {
        this.dialect = dialect;
        return this;
    }

    public KQLTranspiler build() {
        return new KQLTranspiler(kql, resolver, functions, dialect);
    }
}
