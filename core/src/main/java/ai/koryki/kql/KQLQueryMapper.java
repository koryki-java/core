...
public class KQLQueryMapper {
     ...
     *
     * <p>An {@code IdentityHashMap} rather than a {@code HashMap}: the key is the model node
     * itself, and two structurally equal nodes are still two different places in the query. It
     * works today because no bean overrides {@code equals}/{@code hashCode} — this states that
     * assumption instead of relying on it, so adding one later cannot silently make two nodes share
     * a position.
     */
    private final Map<Object, RuleContext> iqlToContext = new java.util.IdentityHashMap<>();

    public KQLQueryMapper(
            LinkResolver resolver, KQLParser.QueryContext script, String description) {
    ...
    private Select toSelect(KQLParser.SelectContext select) {
        ...
        if (select.filterClause() != null) {
            LogicalExpression node =
                    toLogicalNode(aliasToTable, select.filterClause().logical_expression());
    ...
    private void iterateLinks(
            List<KQLParser.LinkContext> links,
            String from,
            Map<String, Source> aliasToTable,
            HashMap<String, List<Join>> joins) {
        for (KQLParser.LinkContext link : links) {

            String f = link.from != null ? link.from.getText() : from;

            Join join = toJoin(link, aliasToTable);
            // Only an unwritten join is inferred. Explicit columns ARE the criterion; looking one
            // up
            ...
            if (join.getCrit() == null && join.getColumns() == null) {
                ...
                // first test for blockId, as visibilityContext may already contain alias, but this
                // is the table itself
    ...
    private LogicalExpression toLogicalNode(
            Map<String, Source> aliasToTable,
            KQLParser.Logical_expressionContext logicalExpression) {
            ...
            return build(
                    ...
                    () ->
                            LogicalExpression.value(
                                    toUnaryLogicalExpression(
                                            aliasToTable,
                                            logicalExpression.unary_logical_expression())));
    ...
    private UnaryLogicalExpression toUnaryLogicalExpression(
            Map<String, Source> aliasToTable,
            KQLParser.Unary_logical_expressionContext unaryLogicalExpressionContext) {

        if (unaryLogicalExpressionContext.PLACEHOLDER() != null) {
            UnaryLogicalExpression bean =
                    build(unaryLogicalExpressionContext, UnaryLogicalExpression::new);
            ...
            bean.setNode(
                    toLogicalNode(
                            aliasToTable, unaryLogicalExpressionContext.logical_expression()));
            return bean;
            ...
            for (int i = 1; i < unaryLogicalExpressionContext.expression().size(); i++) {
                bean.getRight()
                        .add(toExpression(unaryLogicalExpressionContext.expression().get(i)));
            }
            return bean;
    ...
    private Exists toExists(Map<String, Source> aliasToTable, KQLParser.ExistsContext exists) {
        ...
        iterateLinks(exists.link(), from, aToT, joins);

        if (exists.filterClause() != null) {
            LogicalExpression node =
                    toLogicalNode(aToT, exists.filterClause().logical_expression());
            node = Normalizer.normalize(node);
            e.setFilter(node);
        }
        return e;
        ...
        if (expression.select() != null) {
            ...
            if (expression.left == null) {
                ...
                if (inner.getDuration() != null && !inner.isParenthesized()) {
                    Expression bean = build(expression, Expression::new);
                    bean.setDuration(
                            new ai.koryki.iql.query.Duration(
    ...
    private Expression mathFunction(KQLParser.ExpressionContext expression, String name) {
        ...
        f.setFunc(name);
        f.setArguments(
                Arrays.asList(toExpression(expression.left), toExpression(expression.right)));
        bean.setFunction(f);
        return bean;
    }
    ...
    private Expression toExpression(KQLParser.Temporal_literalContext date) {
        ...
        if (date.TIME_STRING() != null) {
            ...
            bean.setLocalDateTime(
                    LocalDateTime.parse(
                            unquoteDouble(date.TIMESTAMP_STRING().getText()), TIMESTAMP_FMT));
    ...
    /**
     * A positioned alias error. These are raised here rather than collected as violations because
     * the mapper cannot build a usable tree without a resolved alias — everything downstream would
     * trip over the same gap. {@link PositionException} carries line/column, so the caller can
     * point at it.
     */
