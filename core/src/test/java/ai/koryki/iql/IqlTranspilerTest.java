...
public class IqlTranspilerTest {
    ...
    public static final Path LOCAL_SQL =
            Path.of("src/test/resources/ai/koryki/core/expected/iql/northwind/sql");
    ...
    private static Path root(Path iql) {
        return iql.startsWith(LOCAL) ? LOCAL : SHARED;
    }

    private static Path sqlRoot(Path iql) {
        return iql.startsWith(LOCAL) ? LOCAL_SQL : EXPECTED;
    }
    ...
    private static void test(Path iql) throws IOException {
        IQLTranspiler transpiler =
                IQLTranspiler.builder(new FileInputStream(iql.toFile()), resolver).build();
    ...
    private static void roundtrip(Query query, String sql) {
        ...
        String sql2 =
                IQLTranspiler.builder(iql2, resolver)
                        .build()
                        .getSql(
                                new SqlQueryRenderer(
                                        DuckdbBaseDialect.INSTANCE, java.time.ZoneId.of("UTC")));
