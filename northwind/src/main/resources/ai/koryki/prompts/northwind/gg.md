...
- Exact colours: `SCALE color TO ['#e45756', '#4c78a8', '#54a24b']` — one per discrete value. Use
  this when the reader names colours or asks for brand colours; a named scheme cannot honour
  "make the bars blue".
- **A bare `TO [...]` binds to the SORTED domain, not to the order the rows arrive in.** So the
  first colour goes to whichever value sorts first, which is almost never the one that was named.
- Highlight ONE value: give the domain too, and the colour lands on the value you name:
  `SCALE color FROM ['Confections'] TO ['#e45756']`
- Colour several deliberately: give the whole domain, in the same order as the range:
  `SCALE color FROM ['Confections', 'Produce'] TO ['#e45756', '#808080']`
  Take the values from RESULT VALUES; do not invent or reorder them.
...
## PLACE — annotations, reference lines and callouts

`PLACE <mark> SETTING <channel> => <value>, …`

Adds a layer that carries its OWN data instead of the shared rows, so it can sit at a coordinate the
result does not contain. Every `SETTING` key is an encoding channel, and its value is a constant.
This is how domain knowledge that is not in the data gets onto the chart — a target, a launch date,
a threshold, a note.

- Reference line across the chart: `PLACE rule SETTING y => 10, color => 'red'`.
- Vertical line at a date: `PLACE rule SETTING x => '2024-06-15'`.
- A text callout: `PLACE text SETTING x => '2024-06-15', y => 120, text => 'Campaign launch'`.
- Use `color` (or `stroke` on a rule) for the annotation's own colour; it is a constant, so it does
  not disturb the `color` scale the data uses.

Several `PLACE` clauses stack. A line and its label are two of them, at the same coordinate:

```
VISUALISE month AS x, revenue AS y
DRAW line
PLACE rule SETTING x => '2024-06-15', color => 'red'
PLACE text SETTING x => '2024-06-15', y => 120, text => 'Campaign launch'
```

When the reader supplies a fact the query cannot know ("a campaign launched on 15 June", "our target
is 100k"), that is a `PLACE`, not a title and not a filter.
...
- Highlighting a value with `SCALE color TO [...]` and no `FROM` — the colours bind to the
  sorted domain, so the wrong bar is highlighted. Name the value in `FROM`.
- Inventing a data value. Where RESULT VALUES is given, the literals in it are the only ones
  the result contains; a value spelled any other way matches no row and highlights nothing.
