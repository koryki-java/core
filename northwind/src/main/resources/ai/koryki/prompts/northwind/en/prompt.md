...
OPTIONAL-Link resolves to OUTER JOIN in SQL. Connected entity may not exist. Place `+` at one end of the link.
    ...
    FIND employees emp, emp VIA reports_to + employees boss
...
When a thing takes **more than one column to identify** — an employee is a last name and a first
name, a person may be a name and a birthdate because two people share the name — join those columns
into **one** result column with `concat` instead of listing them side by side. Two columns for one
thing split it across two headings, and a chart grouped by it gets a pair on its category axis
rather than a person. Give the joined column a header and label it after the thing itself.

    FIND order_details od, od orders o, o employees e
    FETCH concat(e.last_name, ' ', e.first_name) employee "Employee",
          sum(od.unit_price * od.quantity) revenue

This applies to what the query is split BY. Columns that are separately meaningful — a country and
a city a reader may want to sort or filter on one at a time — stay separate columns.
...
A time series needs a **date** on the x axis, not a number. `month_begin`, `year_begin` and
`day_begin` return a date and so give a real time axis with correct spacing:

    FIND orders o,
     o order_details d
    FETCH month_begin(o.order_date) month_start,
     sum(d.unit_price * d.quantity) revenue
    VISUALISE month_start AS x, revenue AS y
    DRAW line

`year_month` returns the number `202401` instead. Read as an axis, December to January is 89 units
rather than one, and every year boundary tears a gap into the trend. `year_month` is a compact,
sortable key for grouping -- not an axis.

`month(o.order_date)` yields 1..12 and folds every January together: that is a seasonal profile,
not a development over time. So for "trend", "development" or "over time", use `month_begin`.
...
A whole query may stand where a value is expected:
...
The inner query is self-contained: it does **not** see the aliases of the outer one. To compare a row
against a value computed per group, use a query-block instead.

`IN` takes a nested query too:

    FIND customers c
    FILTER c.customer_id IN (
        FIND customers c2, c2 orders o2
...
A query-block computes a result once, under a name. In **FIND** it is then used like an entity:

    WITH avgprice AS (
        FIND products p2, p2 categories c2
        FETCH c2.category_name cname, avg(p2.unit_price) avgp
    )
    FIND products p, p categories c, c avgprice a
    FETCH p.product_name, p.unit_price, a.avgp

This is how a row is compared against a value computed per group -- a nested query cannot do it,
because it does not see the outer aliases.
...
**Calendar parts** -- integers, cyclic: `year`, `quarter`, `month`, `week`, `day`, `hour`,
`minute`, `second`, `dayofweek` (Monday = 1), `dayofyear`

**Period start** -- returns a date, and so the one to use for a time axis: `minute_begin`,
`hour_begin`, `day_begin`, `week_begin` (Monday), `month_begin`, `quarter_begin`, `year_begin`

**Period end** -- the last day of the period: `week_end`, `month_end`, `quarter_end`, `year_end`

**Period key:** `year_month` returns the number `202401`. For compact grouping, not as an axis --
see *Time axes*.
...
This list is a selection, not the full set. `kqlFunctions` reports what this deployment really
accepts -- including dialect functions not named here -- so ask it instead of assuming a function is
missing. A function koryki does not know is reported by `validateKQL`.
