...
/**
 * Offline generator for the shared test-module per-dialect SQL goldens ({@code
 * koryki-java/test/.../expected/<dialect>/<schema>/sql/docs/*.sql}). Transpiles every documentation
 * sample to every dialect without a live database — the same {@link SqlQueryRenderer} the engine
 * tests use in their SQL-golden step, so the output is byte-identical. Opt-in, since it writes into
 * the sibling project: run with {@code -Dgoldens.write=true}.
 *
 * <p>A sample is skipped for a dialect when it carries a {@code // ignore=<dialect>} line or fails
 * to transpile (a function the dialect does not support), mirroring the coverage of the hand-run
 * goldens.
 */
    ...
    void generateTestModuleGoldens() throws IOException {
        ...
        for (Map.Entry<String, LinkResolver> se : resolvers.entrySet()) {
            ...
            try (Stream<Path> walk = Files.walk(samplesRoot, FileVisitOption.FOLLOW_LINKS)) {
                for (Path kql :
                        (Iterable<Path>)
                                walk.filter(p -> p.toString().endsWith(".kql"))::iterator) {
                    String kqlText = Files.readString(kql);
                    String relSql =
                            samplesRoot.relativize(kql).toString().replaceFirst("\\.kql$", ".sql");
                    for (Map.Entry<String, SqlDialect> de : dialects.entrySet()) {
                        ...
                        try {
                            sql =
                                    KQLTranspiler.builder(kqlText, se.getValue())
                                            .build()
                                            .getSql(
                                                    new SqlQueryRenderer(
                                                            de.getValue(), ZoneId.of("UTC")));
