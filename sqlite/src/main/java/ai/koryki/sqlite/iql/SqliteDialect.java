...
public class SqliteDialect implements SqlDialect {
    ...
    private static FunctionRenderer buildFunctionRenderer() {
        ...
        r.override(
                "week",
                "((CAST(strftime('%j', date({0}, '-3 days', 'weekday 4')) AS INTEGER) - 1) / 7 + 1)");
        ...
        r.override(
                "right",
                "substr({0}, CASE WHEN {1} <= 0 THEN length({0}) + 1 ELSE max(length({0}) - {1} + 1, 1) END)");
        ...
        r.register(
                new FunctionDefinition("date_trunc", ReturnTypes.ARG1) {
                    ...
                    protected String renderBody(
                        ...
                        return switch (unit) {
                            // No 'start of hour' modifier in SQLite -- strftime and back is the
                            // way.
        ...
        // SQLite spells the row-wise forms max()/min(); with 2+ args they are scalar, not
        // aggregate.
        ...
        r.override(
                "quarter_end",
                "date({0}, 'start of month', printf('%+d months', "
        ...
        // the two readings split the engines five to three. The date() wrappers were what made this
        // count boundaries; julianday on the full
        ...
        // month/year shifts clamp to month-end — SQLite '+N months' overflows (2022-10-31 +1mo →
        // 12-01).
        r.override(
                "month_add",
                "min(date({0}, printf('%+d months', {1})), "
                        + "date({0}, 'start of month', printf('%+d months', {1} + 1), '-1 day'))");
        r.override(
                "year_add",
                "min(date({0}, printf('%+d years', {1})), "
    ...
    private static String quarterBegin(String value) {
        return "date("
                + value
                + ", 'start of month', printf('%+d months', "
                + "-((CAST(strftime('%m', "
                + value
                + ") AS INTEGER) - 1) % 3)))";
    }
     ...
     * <em>down</em> when counting forward and one <em>up</em> when counting backward. Java gets
     * this from integer division on a packed value; SQLite has no such packing, so the two cases
     * are spelled out. {@code raw} is the difference in whole units ignoring the finer field, which
    ...
    /**
     * Translate a canonical KQL format literal to SQLite strftime tokens; non-literals pass
     * through.
     */
     ...
     *
     * <ul>
     *   <li>A DATE_FROM_EPOCH_DAY column is the day count since 1970-01-01.
     ...
     *       so clamp with {@code min(date(base,'+N months'), last-day-of-target-month)} to match
     *       the model. Day/clock parts are plain modifiers; {@code datetime()} is used once a clock
     *       part is present so the result carries a time.
     * </ul>
     */
     ...
     * operands are declared integers. SQLite is dynamically typed: a DECIMAL-declared column
     * holding {@code 2} is stored as an integer, so whether {@code /} truncates depends on the
     * <em>row</em> rather than on the schema. Measured — restricting the cast to declared-integer
     * operands turned a unit cost of 0.1 into 0.0 on the northwind order_details. The declared
     * family cannot decide this here, so it is not consulted.
     */
    @Override
    public String renderDivision(
            String leftSql,
            TypeDescriptor leftType,
            String rightSql,
            TypeDescriptor rightType,
    ...
    public String renderEncodedArithmetic(
        ...
        // Base time-value: an epoch-day column materializes to a date; an EPOCH timestamp to a
        // datetime;
        // a plain date column is used as-is (the enclosing date()/datetime() normalizes it).
        String base =
                leftType != null
                                && CoreTypeEncoding.DATE_FROM_EPOCH_DAY.equals(
                                        leftType.getTypeEncoding())
        ...
        boolean timestampBase =
                leftType != null && CoreTypeFamily.TIMESTAMP.equals(leftType.getTypeFamily());
        ...
        for (Duration.Component c : dur.getComponents()) {
            ...
            switch (c.unit()) {
                ...
                case HOUR -> {
                    mods.add(modifier(v, "hours"));
                    hasClock = true;
                }
                case MINUTE -> {
                    mods.add(modifier(v, "minutes"));
                    hasClock = true;
                }
                case SECOND -> {
                    mods.add(modifier(v, "seconds"));
                    hasClock = true;
                }
                case MILLISECOND -> {
                    mods.add("'" + (v >= 0 ? "+" : "") + (v / 1000.0) + " seconds'");
                    hasClock = true;
                }
            }
        }

        // datetime() (not date()) when a clock part follows OR the base is a timestamp, so the
        // time-of-day
        // is preserved; date() keeps a pure-date result a DATE for a date base + date-only
        // duration.
        ...
        if (months != 0) {
            ...
            String lastDay =
                    fn + base + ", 'start of month', " + monthsModifier(months + 1) + ", '-1 day')";
    ...
    /**
     * SQLite has no {@code EXTRACT(EPOCH ...)}; {@code strftime('%s', ...)} reads epoch-seconds
     * (assuming UTC). An INSTANT is stored as text with a {@code +00} suffix that {@code strftime}
     * cannot parse (only {@code Z} / {@code ±HH:MM}), so normalize it to {@code Z} first.
     */
    @Override
    ...
    /**
     * SQLite has no {@code make_timestamp}; {@code datetime(seconds, 'unixepoch')} → ISO-text
     * TIMESTAMP (UTC).
     */
    @Override
