...
public class SnowflakeDatabase<C extends ResultProcessor<?>> extends JdbcDatabase<C> {
    ...
    public static PrivateKey loadPrivateKey(String filename) throws Exception {
        ...
        // Empty is the more common case than null: when the property is commented out in
        // gradle.properties, Gradle passes an empty value through. Without this check that becomes
        // Paths.get("") -- the working directory -- and the message reads "Is a directory", which
        // points at nothing. Measured; a run failed untraceably on exactly this.
        if (filename == null || filename.isBlank()) {
            throw new IllegalArgumentException(
                    "snowflake.privatekey is not set — it must be the path to the PEM private key."
                            + " Set it in gradle.properties or pass"
                            + " -Dsnowflake.privatekey=/path/to/key.p8");
        }
