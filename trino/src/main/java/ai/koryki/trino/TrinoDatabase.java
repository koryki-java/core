...
/**
 * Vendor base for every Trino connection: pins session state at construction (see docs/TEMPORAL.md,
...
public class TrinoDatabase<P extends ResultProcessor<?>> extends JdbcDatabase<P> {
    ...
    public TrinoDatabase(String name, Connection conn, ZoneId modelZone) {
        ...
        setDecoder(
                new TrinoDecoder(
                        modelZone)); // textual INTERVAL "3-6" / "0 01:02:03.000" -> Interval
    }

    /**
     * SET TIME ZONE is the Trino session command (the JDBC "timezone" connection property is the
     * alternative).
     */
    @Override
    ...
    /**
     * Trino federates: an INSTANT surfaces with the Trino type its backend maps to, not a fixed
     * one. A zone-aware backend (Postgres {@code timestamptz}, Oracle/SQL Server with-time-zone)
     * becomes {@code timestamp with time zone} and reads as an {@link java.time.OffsetDateTime}; a
     * naive backend (e.g. the MariaDB connector) becomes {@code timestamp} (no zone), for which
     * {@code getObject(OffsetDateTime.class)} is unsupported. Branch on the reported type so the
     * read is correct for any backend without knowing which one is wired.
     */
    @Override
